...
# SOLUZIONE: Migliorare il metodo leggi_quantita_da_word per leggere TUTTO
    ...
    def calcola_quantita(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap):
        ...
        # CENA A BUFFET
        if "cena" in servizio_lower or "servito" in servizio_lower:
    ...
    def _calcola_primi_lunch_buffet(self, key, num_pers, voce, voce_lower):
        ...
        peso_primi_base_per_persona = 0.222
    ...
    def _calcola_lunch_buffet(self, key, referenze_cibo, num_pers, servizio_lower):
        ...
        if ha_primi:
            ...
            if is_dolce:
                ...
                peso_per_persona = 0.180  # 180g per salati
                categoria = "SALATI"
            ...
            if is_dolce:
                ...
                peso_per_persona = 0.300  # 300g per salati
                categoria = "SALATI"
        ...
        if tipo_ap == "rinforzato":
            if is_dolce:
                ...
                if ha_primi:
                    ...
                    peso_per_persona = 0.180  # 180g per salati (con primi)
                    ...
                    peso_per_persona = 0.250  # 250g per salati (con dolci, senza primi)
    ...
    def _calcola_cena_a_buffet(self, key, referenze_cibo, num_pers, servizio_lower):
        ...
        if is_dolce:
            ...
            peso_per_persona = 0.080  # 80g per contorni
            categoria = "CONTORNI"
...
# ==============================================================================
    ...
    def _calcola_primi_con_bilanciamento(self, key, num_pers, servizio_lower, tipo_ap, bilanciamento, peso_unitario):
        ...
        if is_servito_service:
            ...
            print(f"   🍝 CENA A BUFFET - peso base primi: 220g per persona")
        ...
        if not is_servito_service:
            ...
            # Cena a buffet non ha bilanciamento
            peso_primi_totale_finale = peso_primi_totale_base
            print(f"   🍝 PRIMI CENA A BUFFET (senza bilanciamento): {peso_primi_totale_finale:.3f} kg")
        ...
        for k in primi_elaborati:
            ...
            if "lasagne" in voce_elaborato:
                if is_servito_service:
                    teglie = max(1, math.ceil(num_pers / 15.0))  # Cena a buffet: più teglie
                else:
                    teglie = max(1, math.ceil(num_pers / 20.0)) if len(self.referenze_primi) > 1 else math.ceil(num_pers / 10.0)
                peso_consumato_elaborati += teglie * 2.0  # 2 kg per teglia
            elif "crespelle" in voce_elaborato:
                if is_servito_service:
                    crespelle = max(12, math.ceil((num_pers * 0.9) / 8.0) * 8)  # Cena a buffet: più crespelle
                else:
                    crespelle = max(8, math.ceil((num_pers * 0.64) / 8.0) * 8) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 2) / 8.0) * 8
                peso_consumato_elaborati += crespelle * 0.080  # 80g per crespella
            elif "cannelloni" in voce_elaborato:
                if is_servito_service:
                    cannelloni = max(12, math.ceil((num_pers * 0.9) / 12.0) * 12)  # Cena a buffet: più cannelloni
                else:
                    cannelloni = max(12, math.ceil((num_pers * 0.64) / 12.0) * 12) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 6.0) * 12
                peso_consumato_elaborati += cannelloni * 0.040  # 40g per cannellone
            elif "rotolini" in voce_elaborato:
                if is_servito_service:
                    rotolini = max(12, math.ceil((num_pers * 0.9) / 12.0) * 12)  # Cena a buffet: più rotolini
                else:
                    rotolini = max(12, math.ceil((num_pers * 0.64) / 12.0) * 12) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 6.0) * 12
                peso_consumato_elaborati += rotolini * 0.045  # 45g per rotolino
            elif "nidi" in voce_elaborato or "rosette" in voce_elaborato:
                if is_servito_service:
                    pezzi = max(18, math.ceil((num_pers * 0.9) / 18.0) * 18)  # Cena a buffet: più pezzi
                else:
                    pezzi = max(18, math.ceil((num_pers * 0.64) / 18.0) * 18) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 9.0) * 18
                peso_consumato_elaborati += pezzi * 0.045  # 45g per pezzo
        ...
        if any(pe in voce_lower for pe in primi_elaborati_keywords):
            ...
            if "lasagne" in voce_lower:
                if is_servito_service:
                    ...
                    print(f"   🍝 Lasagne CENA A BUFFET: {teglie} teglie")
                else:
                    teglie = max(1, math.ceil(num_pers / 20.0)) if len(self.referenze_primi) > 1 else math.ceil(num_pers / 10.0)
                ...
                if is_servito_service:
                    ...
                    print(f"   🥟 Crespelle CENA A BUFFET: {crespelle}")
                else:
                    crespelle = max(8, math.ceil((num_pers * 0.64) / 8.0) * 8) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 2) / 8.0) * 8
                ...
                if is_servito_service:
                    ...
                    print(f"   🥘 Cannelloni CENA A BUFFET: {cannelloni}")
                else:
                    cannelloni = max(12, math.ceil((num_pers * 0.64) / 12.0) * 12) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 6.0) * 12
                ...
                if is_servito_service:
                    rotolini = max(12, math.ceil((num_pers * 0.9) / 12.0) * 12)
                    print(f"   🍝 Rotolini CENA A BUFFET: {rotolini}")
                else:
                    rotolini = max(12, math.ceil((num_pers * 0.64) / 12.0) * 12) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 6.0) * 12
                    print(f"   🍝 Rotolini BILANCIATO: {rotolini}")
                return f"{int(rotolini)} rotolini"
                ...
                if is_servito_service:
                    ...
                    print(f"   🍜 {tipo.title()} CENA A BUFFET: {pezzi}")
                else:
                    pezzi = max(18, math.ceil((num_pers * 0.64) / 18.0) * 18) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 9.0) * 18
    ...
    def _calcola_dolci(self, key, num_pers, servizio_lower, tipo_ap, peso_unitario):
        ...
        # Caso 2: Un bicchierino + altri dolci (rapporto 35%-65%)
            ...
            if "aperitivo" in servizio_lower:
                ...
                peso_dolci_per_persona = 0.130 if ha_primi else 0.120
    ...
    def _calcola_finger_food_con_bilanciamento(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap, peso_unitario, bilanciamento):
        ...
        if use_balancing:
            ...
            if is_lunch:
                if ha_primi:
                    peso_finger_base_per_persona = 0.280  # 280g salato con primi (+100g)
                else:
                    peso_finger_base_per_persona = 0.450  # 450g salato senza primi (+100g)
            ...
            if "coffee" in servizio_lower or "tea" in servizio_lower:
                ...
                peso_salato_per_persona = 0.450  
                print(f"   🍽️ Altri servizi: 373g per persona")
            ...
            print(f"   🍽️ Cena a buffet - Peso target: {peso_target:.3f} kg")
            ...
            if self._va_in_kg(voce):
                ...
                print(f"   🍽️ Cena a buffet finger food: {risultato_pezzi} pezzi")
                return f"{risultato_pezzi} pz"
    ...
    def generate_doc(self):
       ...
       # ========== GENERA INTESTAZIONE (solo se NON aggiungi servizi) ==========
        ...
        # ========== DETERMINA SERVIZIO ==========
        servizi_possibili = ["Coffee Break", "Tea Break", "Lunch Buffet", "Aperitivo a Buffet","Lunch Box","Cena a buffet"]
