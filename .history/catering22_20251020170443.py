...
# SOLUZIONE: Migliorare il metodo leggi_quantita_da_word per leggere TUTTO
    ...
    def _calcola_lunch_buffet(self, key, referenze_cibo, num_pers, servizio_lower):
        ...
        - Finger food salati: distribuzione proporzionale
        - Dolci: distribuzione proporzionale
        """
        voce = self.voce_originale.get(key, key).strip()
        voce_lower = voce.lower()
        ...
        if ha_primi:
            ...
            if is_dolce:
                peso_per_persona = 0.167  # 167g per dolci
                categoria = "DOLCI"
                print(f"   🍽️ Lunch Buffet DOLCI (con primi): 167g per persona")
            else:
                peso_per_persona = 0.200  # 200g per salati (più abbondanti)
                categoria = "SALATI"
                print(f"   🍽️ Lunch Buffet SALATI (con primi): 200g per persona")
        else:
            # SENZA PRIMI: 2 categorie
            if is_dolce:
                peso_per_persona = 0.073  # 73g per dolci
                categoria = "DOLCI"
                print(f"   🍽️ Lunch Buffet DOLCI (senza primi): 73g per persona")
            else:
                peso_per_persona = 0.235  # 235g per salati
                categoria = "SALATI"
                print(f"   🍽️ Lunch Buffet SALATI (senza primi): 235g per persona")
    ...
    def _calcola_aperitivo_buffet(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap):
        ...
        LOGICA:
        LEGGERO (275g totali):
        - Con dolci: 150g salati + 125g dolci
        - Senza dolci: 275g salati

        RINFORZATO (350g totali):
        - Con dolci: 260g salati + 90g dolci
        - Senza dolci: 350g salati
        ...
        ha_salati = any(k not in self.referenze_dolci for k in referenze_cibo)

        # Determina i pesi in base al tipo di aperitivo e presenza dolci
        if tipo_ap == "rinforzato":
            if is_dolce:
                peso_per_persona = 0.090  # 90g per dolci
                categoria = "DOLCI"
                print(f"   🥂 Aperitivo Rinforzato DOLCI: 90g per persona")
            else:
                # Se non ci sono dolci, i salati prendono tutto il peso (350g)
                if not ha_dolci:
                    peso_per_persona = 0.350  # 350g per salati (quando nessun dolce)
                    print(f"   🥂 Aperitivo Rinforzato SALATI (senza dolci): 350g per persona")
                else:
                    peso_per_persona = 0.260  # 260g per salati (con dolci)
                    print(f"   🥂 Aperitivo Rinforzato SALATI (con dolci): 260g per persona")
                categoria = "SALATI"
        else:  # leggero
            if is_dolce:
                peso_per_persona = 0.125  # 125g per dolci
                categoria = "DOLCI"
                print(f"   🥂 Aperitivo Leggero DOLCI: 125g per persona")
            else:
                # Se non ci sono dolci, i salati prendono tutto il peso (275g)
                if not ha_dolci:
                    peso_per_persona = 0.275  # 275g per salati (quando nessun dolce)
                    print(f"   🥂 Aperitivo Leggero SALATI (senza dolci): 275g per persona")
                else:
                    peso_per_persona = 0.150  # 150g per salati (con dolci)
                    print(f"   🥂 Aperitivo Leggero SALATI (con dolci): 150g per persona")
                categoria = "SALATI"
