...
# ==============================================================================
    ...
    def calcola_quantita_menu_extra_dolci_aperitivo_rinforzato(self, voce, persone, servizio_lower, tipo_ap):
        ...
        if "aperitivo" in servizio_lower and "rinforzato" in tipo_ap:
            ...
            # Conta quanti dolci sono selezionati (ESCLUDI finger food salati con "mousse")
            dolci_selezionati = 0
            for key, checkbox in self.selected.items():
                if hasattr(checkbox, 'isChecked') and checkbox.isChecked():
                    voce_check = self.voce_originale.get(key, key.split('|')[-1]).lower()
                    
                    # ESCLUDI finger food salati che contengono "mousse" ma non sono dolci veri
                    if ("bruschettine" in voce_check or "crostini" in voce_check):
                        continue  # Salta i finger food salati
                    
                    # Verifica se è un dolce vero
