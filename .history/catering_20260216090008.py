...
# SOLUZIONE: Migliorare il metodo leggi_quantita_da_word per leggere TUTTO
        ...
        try:
            ...
            def processa_testo(testo):
                ...
                for linea in linee:
                    ...
                    if not linea_strip:
                        # Riga vuota: inseriamo un separatore per evitare unioni di blocchi diversi
                        righe.append("")
                        continue
                        
                    # Se la riga inizia con un numero o con * seguito da numero
                    if re.match(r'^[\*\s\(]*\d+', linea_strip):
                        righe.append(linea_strip)
                    else:
                        # Se non inizia con un numero, ma la riga precedente non è vuota, la uniamo! 
                        # (Risolve il problema di "Polpettine in \n spiedo")
                        if righe and righe[-1] != "":
                            righe[-1] += " " + linea_strip
                        else:
                            righe.append(linea_strip)

            # Leggi i paragrafi standard
            for para in doc.paragraphs:
                processa_testo(para.text)

            # Leggi le tabelle (dove di solito ci sono i menu)
            celle_visitate = set()
            ...
            # 2. Costruiamo una mappa sicura dei cibi (PULITA da parentesi per un matching ultra-flessibile)
            cibi_validi_base = {}
            if hasattr(self, 'voce_originale'):
                for key, nome_orig in self.voce_originale.items():
                    ...
                    if cat in ["cibo", "dolci", "intolleranti"]:
                        # Rimuovi parentesi es. (inox), e asterischi per il confronto testuale
                        base = re.sub(r'\s*\(.*?\)', '', nome_orig)
                        base = base.replace('*', '').strip().lower()
                        # Normalizza eventuali doppi spazi
            ...
            for riga in righe:
                if not riga: continue # salta i separatori vuoti
                ...
                if quantita_val and nome_raw:
                    ...
                    nome_pulito = re.sub(r'\s+', ' ', nome_pulito) # Normalizza doppi spazi
                    
                    # Ignora palesemente i campi di servizio
                    ...
                    # Controllo avanzato: ignora differenze minime come (inox) o spazi in più
                    for cibo_base, cibo_vero in cibi_validi_base.items():
                        if cibo_base == nome_pulito_lower or cibo_base in nome_pulito_lower:
                            is_cibo = True
                            nome_finale = cibo_vero # Usa sempre il nome ufficiale del Database!
                            break
    ...
    def _calcola_primi_lunch_buffet(self, key, num_pers, voce, voce_lower, ha_secondi=False):
        ...
        primi_elaborati_keywords = ["lasagne", "cannelloni", "rosette", "nidi", "crespelle", "rotolini"]
        ...
        for k in primi_elaborati:
            ...
            elif "rotolini" in voce_elaborato:
                rotolini = max(12, math.ceil((num_pers * 0.64) / 12.0) * 12) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 6.0) * 12
                peso_consumato_elaborati += rotolini * 0.045  # 45g per rotolino
        ...
        if is_primo:
            ...
            for k in primi_elaborati:
                ...
                if "lasagne" in voce_elaborato:
                    ...
                    peso_consumato_elaborati += rotolini * 0.045
...
# ==============================================================================
    ...
    def determina_unita_misura(self, voce_originale, quantita):
        ...
        primi_elaborati = ["lasagne", "crespelle", "cannelloni", "nidi", "rosette", "rotolini"]
