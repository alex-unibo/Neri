...
""")
    ...
    def determina_tipo_servizio(self, ordine):
        ...
        if any(k.startswith("cena servita|") and v for k, v in scelte.items()):
            servizi_trovati.append("Cena Servita")

        if any(k.startswith("cena a buffet|") and v for k, v in scelte.items()):
            servizi_trovati.append("Cena a Buffet")
...
class EventBadge(QFrame):
    ...
    def setup_style(self):
        ...
        colori = {
            ...
            "Lunch Box": "#f59e0b",         # Arancio
            "Aperitivo Leggero": "#f59e0b", # Giallo/Arancio
            "Aperitivo Rinforzato": "#ef4444", # Rosso/Arancio
            "Cena Servita": "#ec4899",      # Rosa/Magenta
            "Cena a Buffet": "#8b5cf6",     # Viola
            "Altro": "#6b7280"              # Grigio
        }
...
⏰ {campi.get('Orario allestimento', 'N/A')} - {campi.get('Orario Pronti', 'N/A')}
    ...
    def determina_servizio(self):
        """Determina il tipo di servizio - AGGIORNATO CON CENA A BUFFET"""
        ...
        if any(k.startswith("coffee break|") and v for k, v in scelte.items()):
            ...
            return "Lunch box"
        elif any(k.startswith("cena servita|") and v for k, v in scelte.items()):
            return "Cena Servita"
...
class CalendarioEventi(QWidget):
  ...
  """
    ...
    def determina_servizio_ordine(self, ordine):
        """Determina tipo servizio da ordine - AGGIORNATO CON CENA A BUFFET"""
        scelte = ordine.get("scelte", {})
        ...
        # ========== NUOVO: Verifica Cena Servita e Cena a Buffet ==========
        if any(k.startswith("cena servita|") and v for k, v in scelte.items()):
            servizi_trovati.append("Cena Servita")
...
class CateringApp(QMainWindow):
    REFERENZE_IN_KG = [
            # FINGER FOOD E DOLCI
            "biscotteria", "spicchietti crostate", "trancetti tenerina crema e amerene","Certosino","certosino"
            ...
            "Lombatina di vitello in salsa al rosmarino fresco con mazzetto di fagiolini al bacon",
            "Piccatine al balsamico di Modena con ventaglio di verdure alla griglia",
            "Polpettine alla bolognese in umido con patate e piselli",
            "Scaloppine di vitello alla bolognese con crudo di Parma e fonduta di formaggi", 
            "Sella di maialino brasato al chianti",
            "Spezzatino di vitello spadellato ai funghi porcini",
            "Straccetti di vitello in crema alle punte di asparago",
    ...
    def create_order_info_section(self, parent_layout):
       ...
       # Checkbox "In Fiera" - usa CustomCheckBox per X blu
        ...
        # --- NUOVO: PULSANTE COPIA SELEZIONE ---
        self.copy_btn = QPushButton("📋 Copia Selezione")
        self.copy_btn.setStyleSheet("""
            QPushButton {
                background-color: #f59e0b;  /* Arancione/Giallo scuro */
                color: white;
                font-weight: bold;
                border-radius: 6px;
            }
        ...
        """)
        self.copy_btn.clicked.connect(self.copia_voci_selezionate)
    ...
    def add_service_section(self, parent_layout, tab_name, section_name, items):
        ...
        for i, item in enumerate(items):
            ...
            # NUOVO: Se la voce contiene "cucch.bio" o "forch.bio", collega la funzione di auto-selezione
            if section_type == "cibo" and ("cucch.bio" in item.lower() or "forch.bio" in item.lower()):
                checkbox.toggled.connect(lambda checked, k=key: self.gestisci_selezione_finger_bio(self.selected[k], k))
                print(f"   🍴 Collegata voce bio: {item}")
    ...
    def aggiungi_spiedini_frutta_ai_dolci(self, cibo_cell, num_pers):
        """Aggiunge spiedini frutta nella sezione dolci se selezionati (1 per persona)"""

        # Verifica se "Spiedini frutta" è selezionato
        spiedini_selezionati = False
        for k, checkbox in self.selected.items():
            if (hasattr(checkbox, 'isChecked') and checkbox.isChecked() and
                self.voce_categoria.get(k) == "cibo"):
                voce_lower = self.voce_originale.get(k, "").strip().lower()
                if "spiedini frutta" in voce_lower:
                    spiedini_selezionati = True
                    break

        if spiedini_selezionati:
            # Usa la stessa funzione di stampa degli altri dolci
            quantita_spiedini = f"{num_pers} pz"
            self.stampa_quantita_docx(quantita_spiedini, "Spiedini frutta", cibo_cell, bold=False)
            print(f"🍓 Aggiunti ai dolci: {num_pers} spiedini frutta")
    ...
    def crea_riepilogo_cucina(self, data_target):
        ...
        for i, ordine in enumerate(ordini_giorno, 1):
            ...
            if any(k.startswith("coffee break|") and v for k, v in scelte.items()):
                ...
                servizio_lower = "cena servita"
            elif any(k.startswith("cena a buffet|") and v for k, v in scelte.items()):
                servizio_lower = "cena a buffet"
            elif any(k.startswith("lunch buffet|") and v for k, v in scelte.items()):
            ...
            for k, v in scelte.items():
                if v and self.voce_categoria.get(k) == "cibo":
                    ...
                    # Escludi "Spiedini frutta" dal calcolo del cibo
                    if "spiedini frutta" in voce_lower:
                        continue
...
# SOLUZIONE: Migliorare il metodo leggi_quantita_da_word per leggere TUTTO
    ...
    def leggi_quantita_da_word(self, path_docx):
        ...
        try:
            ...
            def processa_testo(testo):
                if not testo or not testo.strip(): return
            ...
            for riga in righe:
                ...
                if quantita_val and nome_raw:
                    ...
                    if nome_pulito_lower in cibi_validi_esatti:
                        ...
                        # TENTATIVO 2: Ricerca flessibile (Senza parentesi)
                        for cibo_base, cibo_vero in cibi_validi_base.items():
                            if cibo_base == nome_pulito_lower or cibo_base in nome_pulito_lower:
                                is_cibo = True
    ...
    def calcola_bilanciamento_dinamico(self, servizio_lower, tipo_ap, num_pers):
        ...
        # ESCLUDE Lunch Box dal bilanciamento
        is_lunch_box = "lunch box" in servizio_lower or "lunchbox" in servizio_lower
        is_lunch = ("lunch" in servizio_lower or "pranzo" in servizio_lower) and not is_lunch_box
    ...
    def calcola_quantita(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap):
        ...
        GESTIONI SPECIALI PRESERVATE:
        - Torte (kg torta): 1 kg ogni 10 persone con arrotondamento intelligente
        - Taglieri: 1 tagliere ogni 20 persone
        - Miele/Composta per taglieri: stesso numero dei taglieri
        - Grissini con taglieri: 1 per persona
        """
        ...
        # GESTIONE SPECIALE: TORTE
        if "kg torta" in voce_lower:
            print(f"🎂 TORTA: {num_pers} persone → {num_pers/10:.1f} kg")
            kg_torta = num_pers / 10

            # Arrotondamento intelligente: 0-0.25 → arrotonda per difetto, 0.26-0.75 → .5, 0.76-1 → per eccesso
        ...
        # GESTIONE SPECIALE: MACEDONIA MONO+CUCCH.BIO
        if "macedonia mono" in voce_lower or "mono+cucch" in voce_lower:
            print(f"🍉 MACEDONIA: Sempre 1 per persona")

            # Macedonia sempre 1 per persona, indipendentemente dagli altri dolci
            pezzi_macedonia = num_pers
            print(f"   🍉 Macedonia: {num_pers} persone → {pezzi_macedonia} pezzi (1 a testa)")

            return f"{pezzi_macedonia} pz"

        # GESTIONE SPECIALE: TAGLIERI
        if "taglieri" in voce_lower:
            print(f"🧀 TAGLIERI per {voce}:")
            print(f"   - Persone: {num_pers}")

            if "taglieri di salumi medi" in voce_lower:
        ...
        # GESTIONE SPECIALE: GRISSINI CON TAGLIERI
        ...
        if "lunch box" in servizio_lower or "lunchbox" in servizio_lower:
            return self._calcola_lunch_box(key, referenze_cibo, num_pers, servizio_lower)

        # LUNCH BUFFET
        if "lunch" in servizio_lower or "pranzo" in servizio_lower:
            return self._calcola_lunch_buffet(key, referenze_cibo, num_pers, servizio_lower)

        # APERITIVO A BUFFET (leggero o rinforzato)
        ...
        # Fallback temporaneo
        print("   ⚠️ Servizio non ancora implementato, ritorno 1 pezzo")
        return "1 pz"

    def _calcola_coffee_tea_break(self, key, referenze_cibo, num_pers, servizio_lower):
        """
        Calcola quantità per Coffee Break e Tea Break.

        LOGICA:
        - Dolci: 40g per persona
        - Salati: 40g per persona
        - Totale: 80g per persona
        - Separazione tramite self.referenze_dolci
        ...
        for k in referenze_cibo:
            is_dolce_k = k in self.referenze_dolci
            if is_dolce == is_dolce_k:
                voce_k = self.voce_originale.get(k, k).strip()
                dati_k = self.trova_dati_referenza(voce_k, self.dati_referenze, {"difficolta": 1.0, "peso_unitario": 0.03333})
                difficolta_k = dati_k["difficolta"]
                referenze_stessa_categoria.append((k, difficolta_k))
                somma_difficolta += difficolta_k

        num_prodotti = len(referenze_stessa_categoria)
        print(f"   - Prodotti {categoria} selezionati: {num_prodotti}")
        print(f"   - Somma difficoltà: {somma_difficolta}")

        if num_prodotti == 0:
            print("   ⚠️ Nessun prodotto nella categoria, ritorno 1 pezzo")
            return "1 pz"

        # Ottieni difficoltà del prodotto corrente
        ...
        # Calcola peso totale per categoria
        peso_totale_categoria = peso_per_persona * num_pers

        # Calcola quota proporzionale basata sulla difficoltà
        quota_proporzionale = difficolta / somma_difficolta
        quota_questo_prodotto = peso_totale_categoria * quota_proporzionale

        print(f"   - Peso totale {categoria}: {peso_totale_categoria:.3f} kg")
        print(f"   - Difficoltà questo prodotto: {difficolta}")
        print(f"   - Quota proporzionale: {quota_proporzionale*100:.1f}%")
        print(f"   - Peso per questo prodotto: {quota_questo_prodotto:.3f} kg")
        print(f"   - Peso unitario: {peso_unitario:.5f} kg")

        # Determina se va in kg o pezzi
        if self._va_in_kg(voce):
            print(f"   ✅ Risultato: {quota_questo_prodotto:.1f} kg")
            return f"{quota_questo_prodotto:.1f} kg"
        else:
            pezzi = quota_questo_prodotto / peso_unitario
    ...
    def _calcola_primi_lunch_buffet(self, key, num_pers, voce, voce_lower, ha_secondi=False):
        """Calcola quantità primi per lunch buffet con bilanciamento"""
        ...
        peso_primi_totale_finale = peso_primi_base_per_persona * num_pers

        # IDENTIFICA TIPOLOGIA PRIMI
        primi_elaborati_keywords = ["lasagne", "cannelloni", "rosette", "nidi", "crespelle", "rotolini", "intrighi"]
        ...
        for k in self.referenze_primi:
            ...
            if any(pe in voce_primo for pe in primi_elaborati_keywords):
                primi_elaborati.append(k)
            elif any(pf in voce_primo for pf in primi_freddi_keywords):
        ...
        for k in primi_elaborati:
            ...
            elif "rotolini" in voce_elaborato or "intrighi" in voce_elaborato:
                quantita_pz = max(12, math.ceil((num_pers * 0.64) / 12.0) * 12) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 6.0) * 12
                peso_consumato_elaborati += quantita_pz * 0.045  # 45g a pezzo
        ...
        if any(pe in voce_lower for pe in primi_elaborati_keywords):
            ...
            primi_crudi = [
                ...
                "gramigna gratinata con salsiccia dolce, porri e zafferano(inox)*",
            ...
            print(f"   🍝 Primo {tipo_primo}: {peso_finale:.1f} kg")

            return f"{peso_finale:.1f} kg"

    def _calcola_lunch_buffet(self, key, referenze_cibo, num_pers, servizio_lower):
        """
        Calcola quantità per Lunch Buffet con logica bilanciamento dinamico.

        LOGICA COMPLETA (6 COMBINAZIONI):

        1. SENZA PRIMI (solo finger + dolci):
           - Finger food: 330g | Dolci: 80g | TOTALE: 410g

        2. CON PRIMI + FINGER (no secondo/contorno):
           - Primi: 180g | Finger food: 190g | Dolci: 80g | TOTALE: 450g

        3. CON PRIMI + FINGER + SOLO CONTORNO:
           - Primi: 180g | Contorni: 90g | Finger food: 180g | Dolci: 80g | TOTALE: 530g

        4. CON PRIMI + FINGER + SOLO SECONDO:
           - Primi: 100g | Secondi: 150g | Finger food: 150g | Dolci: 80g | TOTALE: 480g

        5. CON PRIMI + FINGER + SECONDO + CONTORNO:
           - Primi: 100g | Secondi: 150g | Contorni: 90g | Finger food: 150g | Dolci: 80g | TOTALE: 570g

        6. SOLO SECONDO/CONTORNO (senza primi):
           - Secondi: 150g | Contorni: 90g | Finger food: 150g | Dolci: 80g

        REGOLE:
        - Dolci: sempre 80g
        - Secondi: sempre 150g
        - Contorni: sempre 90g
        - Primi: 180g (senza secondi) o 100g (con secondi)
        - Finger food: variabile in base alla combinazione
        - Primi elaborati (lasagne, crespelle, ecc.): calcolo fisso a pezzi
        - Primi normali/freddi: peso rimanente dopo elaborati
        - Distribuzione PROPORZIONALE basata sul coefficiente di difficoltà
        """
        voce = self.voce_originale.get(key, key).strip()
        voce_lower = voce.lower()

        # Verifica se ci sono primi, secondi, contorni selezionati
        ha_primi = any(k in self.referenze_primi for k in referenze_cibo)
        ha_secondi = any(k in self.referenze_secondi for k in referenze_cibo) if hasattr(self, 'referenze_secondi') else False
        ha_contorni = any(k in self.referenze_contorni for k in referenze_cibo) if hasattr(self, 'referenze_contorni') else False
        ...
        if is_secondo:
            ...
            # FINGER FOOD SALATI - logica complessa in base alle combinazioni
            if ha_primi:
                # Con primi piatti
                if ha_secondi:
                    peso_per_persona = 0.100 # Con primi + secondo (con o senza contorno)
                    categoria = "SALATI"
                    dettaglio = "con secondo" if not ha_contorni else "con secondo e contorno"
                    print(f"   🍽️ Lunch Buffet SALATI (primi + {dettaglio}): 150g per persona")
                elif ha_contorni:
                    peso_per_persona = 0.170  # Con primi + solo contorno
                    categoria = "SALATI"
                    print(f"   🍽️ Lunch Buffet SALATI (primi + contorno): 180g per persona")
                else:
                    peso_per_persona = 0.190  # Con primi (no secondo/contorno)
                    categoria = "SALATI"
                    print(f"   🍽️ Lunch Buffet SALATI (solo primi): 190g per persona")
            else:
                # Senza primi piatti
                if ha_secondi or ha_contorni:
                    peso_per_persona = 0.150  # Con secondo/contorno (no primi)
                    categoria = "SALATI"
                    dettaglio = []
                    if ha_secondi:
                        dettaglio.append("secondo")
                    if ha_contorni:
                        dettaglio.append("contorno")
                    print(f"   🍽️ Lunch Buffet SALATI (solo {' e '.join(dettaglio)}): 150g per persona")
                else:
                    peso_per_persona = 0.330  # Solo finger + dolci
                    categoria = "SALATI"
                    print(f"   🍽️ Lunch Buffet SALATI (solo finger food): 330g per persona")
        ...
        for k in referenze_cibo:
            ...
            k_is_accompagnamento = k in self.referenze_accompagnamenti

            # Verifica se k appartiene alla stessa categoria del prodotto corrente
            # Saltiamo i primi e accompagnamenti (già gestiti separatamente)
            if k_is_primo or k_is_accompagnamento:
                continue
            ...
            if is_dolce and k_is_dolce:
                ...
                stessa_categoria = True
            elif not is_dolce and not is_secondo and not is_contorno and not k_is_dolce and not k_is_secondo and not k_is_contorno:
                # Salati (finger food): solo se né prodotto corrente né k sono dolci/secondi/contorni
                stessa_categoria = True
        ...
        # GESTIONE SALATI E DOLCI (i primi sono già gestiti sopra)
        if self._va_in_kg(voce):
            print(f"   ✅ Risultato: {quota_questo_prodotto:.1f} kg")
            return f"{quota_questo_prodotto:.1f} kg"
        else:
    ...
    def _calcola_aperitivo_buffet(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap):
        """
        Calcola quantità per Aperitivo a Buffet (Leggero o Rinforzato).

        LOGICA:
        LEGGERO (260g totali):
        - Con dolci: 210g salati + 50g dolci
        - Senza dolci: 260g salati
        - NON supporta primi piatti

        RINFORZATO (330g totali):
        - Con dolci: 250g salati + 80g dolci
        - Senza dolci: 330g salati
        - SUPPORTA primi piatti (calcolo identico a Lunch Buffet)

        - Separazione tramite self.referenze_dolci e self.referenze_primi
        - Distribuzione PROPORZIONALE basata sul coefficiente di difficoltà
        """
        voce = self.voce_originale.get(key, key).strip()
        voce_lower = voce.lower()

        # ========== GESTIONE PRIMI (solo per Aperitivo Rinforzato) ==========
        is_primo = key in self.referenze_primi

        if is_primo:
            if tipo_ap == "rinforzato":
                # Usa lo stesso calcolo del Lunch Buffet per i primi
                return self._calcola_primi_lunch_buffet(key, num_pers, voce, voce_lower)
            else:
                # Aperitivo leggero non supporta primi
                print(f"   ⚠️ Aperitivo Leggero non supporta primi piatti")
                return "0 kg"
        ...
        # Controlla se ci sono dolci e primi selezionati
        ha_dolci = any(k in self.referenze_dolci for k in referenze_cibo)
        ha_salati = any(k not in self.referenze_dolci and k not in self.referenze_primi for k in referenze_cibo)
        ha_primi = any(k in self.referenze_primi for k in referenze_cibo)

        # Determina i pesi in base al tipo di aperitivo e presenza dolci/primi
        if tipo_ap == "rinforzato":
            if is_dolce:
                peso_per_persona = 0.080  # 80g per dolci (sempre)
                categoria = "DOLCI"
                print(f"   🥂 Aperitivo Rinforzato DOLCI: 80g per persona")
            else:
                # Pesi dei salati variano in base alla presenza di primi
                if ha_primi:
                    # Con primi: stesso comportamento del lunch buffet
                    peso_per_persona = 0.250 
                    print(f"   🥂 Aperitivo Rinforzato SALATI (con primi): 180g per persona")
                elif not ha_dolci:
                    peso_per_persona = 0.330  # 330g per salati (senza dolci e senza primi)
                    print(f"   🥂 Aperitivo Rinforzato SALATI (senza dolci, senza primi): 330g per persona")
                else:
                    peso_per_persona = 0.330  # 250g per salati (con dolci, senza primi)
                    print(f"   🥂 Aperitivo Rinforzato SALATI (con dolci, senza primi): 250g per persona")
                categoria = "SALATI"
        else:  # leggero
            if is_dolce:
                peso_per_persona = 0.050  # 50g per dolci
                categoria = "DOLCI"
                print(f"   🥂 Aperitivo Leggero DOLCI: 50g per persona")
            else:
                # Se non ci sono dolci, i salati prendono tutto il peso (260g)
                if not ha_dolci:
                    peso_per_persona = 0.260  # 260g per salati (quando nessun dolce)
                    print(f"   🥂 Aperitivo Leggero SALATI (senza dolci): 260g per persona")
                else:
                    peso_per_persona = 0.155  # 210g per salati (con dolci)
                    print(f"   🥂 Aperitivo Leggero SALATI (con dolci): 210g per persona")
                categoria = "SALATI"
    ...
    def _calcola_cena_a_buffet(self, key, referenze_cibo, num_pers, servizio_lower):
        """
        Calcola quantità per Cena a Buffet.

        LOGICA:
        - Primi: calcolo come Lunch Buffet (usa _calcola_primi_lunch_buffet)
        - Secondi: 150g per persona
        - Contorni: 80g per persona
        - Dolci: 80g per persona
        - Accompagnamenti: pane, grissini, bocconcini (calcolo specifico)
        ...
        # ========== GESTIONE ACCOMPAGNAMENTI ==========
        accompagnamenti_keywords = ["pane a fette", "grissini", "bocconcini alle olive"]
        is_accompagnamento = any(keyword in voce_lower for keyword in accompagnamenti_keywords)

        if is_accompagnamento:
            # Usa il calcolo specifico per accompagnamenti
            return self._calcola_accompagnamenti(key, num_pers, servizio_lower)
        ...
        # Determina pesi per persona
        if is_dolce:
            peso_per_persona = 0.080  # 80g per dolci
            categoria = "DOLCI"
            print(f"   🍰 Cena a Buffet DOLCI: 80g per persona")
        elif is_secondo:
            peso_per_persona = 0.150  # 150g per secondi
            categoria = "SECONDI"
            print(f"   🍖 Cena a Buffet SECONDI: 150g per persona")
        elif is_contorno:
            peso_per_persona = 0.090  # 80g per contorni
            categoria = "CONTORNI"
            print(f"   🥗 Cena a Buffet CONTORNI: 80g per persona")
        else:
            # Default: tratta come salato generico
            peso_per_persona = 0.150
            categoria = "SALATI"
            print(f"   🍽️ Cena a Buffet SALATI: 150g per persona")

        # Raccogli prodotti della stessa categoria con le loro difficoltà (escludendo i primi)
        referenze_stessa_categoria = []
        somma_difficolta = 0

        for k in referenze_cibo:
            is_primo_k = k in self.referenze_primi

            # Salta i primi (gestiti separatamente)
            if is_primo_k:
                continue

            # Verifica se k appartiene alla stessa categoria
            k_is_dolce = k in self.referenze_dolci
            k_is_secondo = k in self.referenze_secondi if hasattr(self, 'referenze_secondi') else False
            k_is_contorno = k in self.referenze_contorni if hasattr(self, 'referenze_contorni') else False
    ...
    def _calcola_cena_servita(self, key, referenze_cibo, num_pers, servizio_lower):
        ...
        LOGICA:
        - Antipasto: 1 pz a persona + 15% (distribuzione equa tra antipasti)
        - Primi: 250g per persona (distribuzione proporzionale basata su difficoltà)
        - Secondi Serviti: 200g per persona (distribuzione proporzionale)
        - Contorni: 150g per persona (distribuzione proporzionale)
        - Dolci: 80g per persona (distribuzione proporzionale)
        - Accompagnamenti: pane, grissini, bocconcini (calcolo specifico)
        ...
        # ========== GESTIONE ACCOMPAGNAMENTI ==========
        accompagnamenti_keywords = ["pane a fette", "grissini", "bocconcini alle olive", "mini crescentine fritte", "crescenta a dadini", "tigelle"]
        is_accompagnamento = any(keyword in voce_lower for keyword in accompagnamenti_keywords)
        ...
        # ========== GESTIONE PRIMI PIATTI ==========
        # I primi per Cena Servita: 250g per persona (diverso dal Lunch Buffet)
        is_primo = "bis di primi piatti caldi|" in key.lower()

        if is_primo:
            print(f"   🍝 Cena Servita PRIMI: 250g per persona")

            # Raccogli tutti i primi selezionati
            primi_selezionati = [k for k in referenze_cibo if "bis di primi piatti caldi|" in k.lower()]
            num_primi = len(primi_selezionati)

            if num_primi == 0:
                print("   ⚠️ Nessun primo selezionato, ritorno 1 pezzo")
                return "1 pz"

            # IDENTIFICA TIPOLOGIA PRIMI (logica elaborati)
            import math
            primi_elaborati_keywords = ["lasagne", "cannelloni", "rosette", "nidi", "crespelle", "rotolini", "intrighi"]
            primi_freddi_keywords = ["cous cous", "orecchiette", "mezze penne", "mezze maniche",
                                    "insalata di riso", "insalatina di farro", "gnocchetti sardi con carpaccio",
            ...
            # LOGICA CENA SERVITA: 1 porzione a persona + 1 porzione extra per ogni primo
            # Esempio con 40 persone:
            # - Crespelle: 40 + 1 = 41 crespelle → arrotondato a multipli di 8 = 48 crespelle
            # - Lasagne: (40 + 1) / 20 = 2.05 teglie → arrotondato per eccesso = 3 teglie
            # Esempio con 41 persone:
            # - Lasagne: (41 + 1) / 20 = 2.1 teglie → arrotondato per eccesso = 3 teglie

            print(f"   🍝 CALCOLO PRIMI CENA SERVITA:")
            print(f"      - Persone: {num_pers}")
            print(f"      - Primi selezionati: {num_primi}")
            print(f"      - Formula: {num_pers} persone + 1 porzione extra")

            # CALCOLA PESO CONSUMATO DA PRIMI ELABORATI (per calcolo peso rimanente)
            peso_consumato_elaborati = 0

            for k in primi_elaborati:
                voce_elaborato = self.voce_originale.get(k, "").lower()
                porzioni_primo = num_pers + 1  # num_pers + 1 extra

                if "lasagne" in voce_elaborato:
                    # 1 teglia = 20 porzioni
                    teglie = math.ceil(porzioni_primo / 20.0)
                    peso_consumato_elaborati += teglie * 2.0
                    print(f"      - Lasagne: {porzioni_primo} porz → {teglie} teglie")
                elif "crespelle" in voce_elaborato:
                    # 1 crespella = 1 porzione, arrotonda a multipli di 8
                    crespelle = math.ceil(porzioni_primo / 8.0) * 8
                    peso_consumato_elaborati += crespelle * 0.080
                    print(f"      - Crespelle: {porzioni_primo} porz → {crespelle} crespelle")
                elif "cannelloni" in voce_elaborato:
                    # 1 cannellone = 1 porzione, arrotonda a multipli di 12
                    cannelloni = math.ceil(porzioni_primo / 12.0) * 12
                    peso_consumato_elaborati += cannelloni * 0.040
                    print(f"      - Cannelloni: {porzioni_primo} porz → {cannelloni} cannelloni")
                elif "rotolini" in voce_elaborato or "intrighi" in voce_elaborato:
                # 1 pezzo = 1 porzione, arrotonda a multipli di 12
                    quantita_pz = math.ceil(porzioni_primo / 12.0) * 12
                    peso_consumato_elaborati += quantita_pz * 0.045
                elif "nidi" in voce_elaborato or "rosette" in voce_elaborato:
                    # 1 nido/rosetta = 1 porzione, arrotonda a multipli di 18
                    pezzi = math.ceil(porzioni_primo / 18.0) * 18
                    peso_consumato_elaborati += pezzi * 0.045
                    tipo = 'nidi' if 'nidi' in voce_elaborato else 'rosette'
                    print(f"      - {tipo.capitalize()}: {porzioni_primo} porz → {pezzi} {tipo}")

            # Peso totale per primi: 250g per persona
            peso_primi_totale = 0.250 * num_pers
            ...
            # RESTITUISCI IL RISULTATO PER IL PRIMO CORRENTE
            if any(pe in voce_lower for pe in primi_elaborati_keywords):
                # PRIMI ELABORATI: calcola con formula num_pers + 1
                porzioni_primo = num_pers + 1

                if "lasagne" in voce_lower:
                    # 1 teglia = 20 porzioni
                    teglie = math.ceil(porzioni_primo / 15.0)
                    print(f"   🍝 Lasagne: {porzioni_primo} porz → {teglie} teglie")
                    return f"{int(teglie)} lasagne"
                elif "crespelle" in voce_lower:
                    # 1 crespella = 1 porzione, arrotonda a multipli di 8
                    ...
                    quantita_pz = math.ceil(porzioni_primo / 12.0) * 12
                    tipo_nome = "intrighi" if "intrighi" in voce_lower else "rotolini"
                    print(f" 🍝 {tipo_nome.title()}: {porzioni_primo} porz → {quantita_pz} {tipo_nome}")
                    return f"{int(quantita_pz)} {tipo_nome}"
                elif "nidi" in voce_lower or "rosette" in voce_lower:
                    tipo = 'nidi' if 'nidi' in voce_lower else 'rosette'
                    # 1 nido/rosetta = 1 porzione, arrotonda a multipli di 18
                ...
                # PRIMI FREDDI O NORMALI: usa peso rimanente
                if self._va_in_kg(voce):
                    print(f"   ✅ Risultato: {peso_per_primo_non_elaborato:.1f} kg")
                    return f"{peso_per_primo_non_elaborato:.1f} kg"
                else:
                    dati = self.trova_dati_referenza(voce, self.dati_referenze, {"difficolta": 1.0, "peso_unitario": 0.03333})
                    peso_unitario = dati["peso_unitario"]
                    pezzi = peso_per_primo_non_elaborato / peso_unitario
                    pezzi_finali = max(1, int(round(pezzi)))
                    print(f"   - Pezzi calcolati: {pezzi:.1f} → {pezzi_finali}")
                    print(f"   ✅ Risultato: {pezzi_finali} pz")
                    return f"{pezzi_finali} pz"
        ...
        # ========== DETERMINA CATEGORIA ==========
        is_antipasto = "antipasto|" in key.lower()
        is_secondo = "secondi piatti serviti|" in key.lower()
        is_contorno = "contorni|" in key.lower()
        is_dolce = "bis di dolci|" in key.lower()

        # ========== GESTIONE SPECIALE ANTIPASTO ==========
        # Gli antipasti hanno logica diversa: 1 pz a persona + 15%
        if is_antipasto:
            print(f"   🍽️ Cena Servita ANTIPASTO: 1 pz per persona + 15%")

            # Conta quanti antipasti sono selezionati
            num_antipasti = sum(1 for k in referenze_cibo if "antipasto|" in k.lower())

            if num_antipasti == 0:
                print("   ⚠️ Nessun antipasto selezionato, ritorno 1 pezzo")
                return "1 pz"

            # Calcolo: (num_pers / num_antipasti) * 1.15
            pezzi_base = num_pers / num_antipasti
            pezzi_con_bis = pezzi_base * 1.15  # +15%
            pezzi_finali = max(1, int(round(pezzi_con_bis)))

            print(f"   - Numero antipasti selezionati: {num_antipasti}")
            print(f"   - Pezzi base per antipasto: {pezzi_base:.1f}")
            print(f"   - Con 15% bis: {pezzi_con_bis:.1f}")
            print(f"   ✅ Risultato: {pezzi_finali} pz")

            return f"{pezzi_finali} pz"

        # Determina pesi per persona (per le altre categorie)
        if is_secondo:
            peso_per_persona = 0.200  # 200g per secondi
            categoria = "SECONDI SERVITI"
            print(f"   🍖 Cena Servita SECONDI: 200g per persona")
        elif is_contorno:
            peso_per_persona = 0.100 # 150g per contorni
            categoria = "CONTORNI"
            print(f"   🥗 Cena Servita CONTORNI: 150g per persona")
        elif is_dolce:
            peso_per_persona = 0.100  # 80g per dolci
            categoria = "DOLCI"
            print(f"   🍰 Cena Servita DOLCI: 80g per persona")
        else:
            # Default: tratta come antipasto
            peso_per_persona = 0.050
            categoria = "ALTRO"
            print(f"   ⚠️ Cena Servita ALTRO: 50g per persona (default)")
        ...
        for k in referenze_cibo:
            # Salta i primi (gestiti separatamente)
            if "bis di primi piatti caldi|" in k.lower():
                continue

            # Verifica se k appartiene alla stessa categoria
            k_is_antipasto = "antipasto|" in k.lower()
            k_is_secondo = "secondi piatti serviti|" in k.lower()
            k_is_contorno = "contorni|" in k.lower()
            k_is_dolce = "bis di dolci|" in k.lower()

            stessa_categoria = False
            if is_antipasto and k_is_antipasto:
                stessa_categoria = True
            elif is_secondo and k_is_secondo:
                stessa_categoria = True
            elif is_contorno and k_is_contorno:
                stessa_categoria = True
            elif is_dolce and k_is_dolce:
                stessa_categoria = True
            elif not is_antipasto and not is_secondo and not is_contorno and not is_dolce and not k_is_antipasto and not k_is_secondo and not k_is_contorno and not k_is_dolce:
                stessa_categoria = True
    ...
    def _calcola_lunch_box(self, key, referenze_cibo, num_pers, servizio_lower):
        ...
        LOGICA:
        - Usa calcola_quantita_menu_extra_simple per calcolare le quantità
        - Estrae solo il numero totale dal formato (totale)per_persona
        - Ritorna solo il numero totale (es: "75 pz" invece di "(75)3 pz")

        Il Lunch Box usa la stessa logica dei menu intolleranti ma mostra solo il totale.
        """
        voce = self.voce_originale.get(key, key).strip()

        # Ottieni il numero di persone dal widget Lunch Box
        # (di solito è lo stesso di num_pers, ma potrebbe essere diverso)
        persone_lunch_box = num_pers

        # Verifica se esiste il widget per Lunch Box
        if hasattr(self, 'menu_extra_widgets') and 'Lunch Box' in self.menu_extra_widgets:
            persone_lunch_box = self.menu_extra_widgets['Lunch Box'].value()
            print(f"   📦 Lunch Box widget: {persone_lunch_box} persone")

        # Usa la funzione per menu extra con il servizio principale
        risultato = self.calcola_quantita_menu_extra_simple(voce, persone_lunch_box, servizio_lower)

        print(f"   📦 Lunch Box risultato grezzo: {risultato}")

        # Estrae solo il numero totale dal formato (totale)per_persona
        # Es: "(75)3" -> "75"
        import re
        match = re.match(r'\((\d+)\)', risultato)
        if match:
            totale = match.group(1)
            risultato_finale = f"{totale} pz"
            print(f"   📦 Lunch Box estratto totale: {risultato_finale}")
            return risultato_finale

        # Se non è nel formato (totale)per_persona, ritorna come è
        # (potrebbe essere già un numero semplice)
        if "pz" not in risultato:
            risultato_finale = f"{risultato} pz"
        else:
            risultato_finale = risultato

        print(f"   📦 Lunch Box finale: {risultato_finale}")
        return risultato_finale
...
# ==============================================================================
    ...
    def _calcola_accompagnamenti(self, key, num_pers, servizio_lower):
        ...
        # LOGICA SPECIALE PER CENA SERVITA
        is_cena_servita = "cena servita" in servizio_lower

        # Calcolo specifico per ogni accompagnamento
        if "grissini" in voce.lower():
            if is_cena_servita:
                # Cena Servita: 10 grissini ogni 10 persone
                pezzi = int((num_pers / 10.0) * 10)
                print(f"   🥖 Grissini (Cena Servita): {num_pers} persone → 10 ogni 10 pers = {pezzi} pezzi")
                return f"{pezzi} pz"
            else:
                # Logica normale per altri servizi
            ...
            if is_cena_servita:
                # Cena Servita: 10 bocconcini ogni 10 persone
                pezzi = int((num_pers / 10.0) * 10)
                print(f"   🫒 Bocconcini alle olive (Cena Servita): {num_pers} persone → 10 ogni 10 pers = {pezzi} pezzi")
                return f"{pezzi} pz"
            else:
                # Logica normale per altri servizi
                pezzi = int(num_pers * 0.5)
            ...
            if is_cena_servita:
                # Cena Servita: 1 pane ogni 10 persone
                pz_finali = num_pers / 10.0
                print(f"   🍞 Pane a fette (Cena Servita): {num_pers} persone → 1 ogni 10 pers = {pz_finali:.1f} pz")
    ...
    def gestisci_selezione_tagliata_frutta(self, checkbox, key):
        ...
        print("🍉 Tagliata di frutta selezionata, cerco di autoselezionare 'Acc x porz frutta'")
        
        # Cerca la checkbox "Acc x porz frutta" nel dizionario delle checkbox e attivala
        trovato = False
        for k, cb in self.selected.items():
            voce_lower = self.voce_originale.get(k, "").lower()
            # Intercetta la voce aggiunta in Optionals > Accessori
            if "acc x porz frutta" in voce_lower:
                cb.setChecked(True)
                print(f"✅ Autoselezionato accessorio: {self.voce_originale.get(k)}")
                trovato = True
                ...
                if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                    ...
                    if len(parti_k) >= 2:
                        ...
                        if sezione_accessorio != sezione_primo:
                            # ECCEZIONE: Se il primo è in "Aperitivo Rinforzato", cerca accessori anche in "Aperitivo a Buffet"
                            if sezione_primo == "aperitivo rinforzato" and sezione_accessorio == "aperitivo a buffet":
                                print(f"   🔗 Collegamento Aperitivo Rinforzato → Aperitivo a Buffet")
                            else:
                                continue  # Salta accessori di altre sezioni
                    else:
                        continue
                ...
                for k, cb in self.selected.items():
                    if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                        ...
                        if len(parti_k) >= 2:
                            sezione_accessorio = parti_k[0].lower()
                            if sezione_accessorio != sezione_primo:
                                # ECCEZIONE: Se il primo è in "Aperitivo Rinforzato", deseleziona accessori anche in "Aperitivo a Buffet"
                                if sezione_primo == "aperitivo rinforzato" and sezione_accessorio == "aperitivo a buffet":
                                    print(f"   🔗 Deselezione Aperitivo Rinforzato → Aperitivo a Buffet")
                                else:
                    ...
                    if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                        ...
                        if len(parti_k) >= 2:
                            ...
                            if sezione_accessorio != sezione_primo:
                                # ECCEZIONE: Se il primo è in "Aperitivo Rinforzato", deseleziona accessori anche in "Aperitivo a Buffet"
                                if sezione_primo == "aperitivo rinforzato" and sezione_accessorio == "aperitivo a buffet":
                                    print(f"   🔗 Deselezione Scaldavivande Aperitivo Rinforzato → Aperitivo a Buffet")
                                else:
                                    continue
                        else:
                            continue
    ...
    # ==============================================================================
    # GESTIONE AUTO-SELEZIONE "Cucch. e forch. Bio x finger"
    # ==============================================================================

    def gestisci_selezione_finger_bio(self, checkbox, key_finger):
        """Quando viene selezionata una voce con cucch.bio o forch.bio, auto-seleziona 'Cucch. e forch. Bio x finger'"""

        voce_finger = self.voce_originale.get(key_finger, "").lower()

        print(f"🍴 Gestione finger bio: {voce_finger[:50]}...")

        # ========== ESTRAI LA SEZIONE DELLA VOCE ==========
        parti_key = key_finger.split('|')
        if len(parti_key) >= 2:
            sezione_finger = parti_key[0].lower()
            print(f"   🔍 Sezione finger: '{sezione_finger}'")
        else:
            print(f"   ⚠️ Formato key non riconosciuto: {key_finger}")
            return

        if checkbox.isChecked():
            # SELEZIONE: attiva "Cucch. e forch. Bio x finger" nella stessa sezione
            for k, cb in self.selected.items():
                if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                    ...
                    if len(parti_k) >= 2:
                        sezione_accessorio = parti_k[0].lower()
                        if sezione_accessorio != sezione_finger:
                            # ECCEZIONE: Se il finger è in "Aperitivo Rinforzato", cerca accessori anche in "Aperitivo a Buffet"
                            if sezione_finger == "aperitivo rinforzato" and sezione_accessorio == "aperitivo a buffet":
                                print(f"   🔗 Collegamento Aperitivo Rinforzato → Aperitivo a Buffet (Bio)")
                            else:
                                continue  # Salta accessori di altre sezioni
                    else:
                        continue

                    voce_accessorio = self.voce_originale.get(k, "")

                    # Cerca "Cucch. e forch. Bio x finger"
                    if "cucch" in voce_accessorio.lower() and "forch" in voce_accessorio.lower() and "bio" in voce_accessorio.lower() and "finger" in voce_accessorio.lower():
                        cb.setChecked(True)
                        print(f"✅ Auto-selezionato: {voce_accessorio} (sezione: {sezione_accessorio})")
        else:
            # DESELEZIONE: controlla se ci sono altre voci con bio selezionate NELLA STESSA SEZIONE
            altre_voci_bio_selezionate = False

            for k, cb in self.selected.items():
                if k != key_finger and hasattr(cb, 'isChecked') and cb.isChecked():
                    categoria = self.voce_categoria.get(k)
                    if categoria == "cibo":
                        ...
                        parti_k = k.split('|')
                        if len(parti_k) >= 2:
                            sezione_altra_voce = parti_k[0].lower()
                            if sezione_altra_voce != sezione_finger:
                                continue  # Considera solo voci della stessa sezione
                        else:
                            continue

                        voce_altra = self.voce_originale.get(k, "").lower()
                        # Controlla se ha cucch.bio o forch.bio
                        if "cucch.bio" in voce_altra or "forch.bio" in voce_altra:
                            altre_voci_bio_selezionate = True
                            break

            # Deseleziona solo se non ci sono altre voci bio NELLA STESSA SEZIONE
            if not altre_voci_bio_selezionate:
                for k, cb in self.selected.items():
                    if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                        ...
                        if len(parti_k) >= 2:
                            sezione_accessorio = parti_k[0].lower()
                            if sezione_accessorio != sezione_finger:
                                # ECCEZIONE: Se il finger è in "Aperitivo Rinforzato", deseleziona accessori anche in "Aperitivo a Buffet"
                                if sezione_finger == "aperitivo rinforzato" and sezione_accessorio == "aperitivo a buffet":
                                    print(f"   🔗 Deselezione Aperitivo Rinforzato → Aperitivo a Buffet (Bio)")
                                else:
                                    continue  # Deseleziona solo accessori della stessa sezione
                        else:
                            continue
                        ...
                        if "cucch" in voce_accessorio.lower() and "forch" in voce_accessorio.lower() and "bio" in voce_accessorio.lower() and "finger" in voce_accessorio.lower():
                            cb.setChecked(False)
                            print(f"❌ Auto-deselezionato: {voce_accessorio} (sezione: {sezione_accessorio})")
    ...
    def determina_unita_misura(self, voce_originale, quantita):
        ...
        primi_elaborati = ["lasagne", "crespelle", "cannelloni", "nidi", "rosette", "rotolini", "intrighi"]
        ...
        if isinstance(quantita, str):
            if "kg" in quantita.lower():
                ...
                return quantita  # Già formattato correttamente (es. "103 pz")
            elif any(x in quantita.lower() for x in ["lasagne", "crespelle", "cannelloni", "nidi", "rosette", "rotolini", "porzioni"]):
                return quantita  # Già formattato correttamente
            else:
        ...
        referenze_in_kg = [
            ...
            "rustici mix", "parmigianini", "pasticceria", "crescenta a dadini","certosino","Certosino",
            ...
            "mozzarelle", "polpettine prosciutto e ricotta","trancetti tenerina cioccolato",
    ...
    def stampa_quantita_docx(self, quant, voce, cella, bold=False):
        ...
        primi_elaborati = ["lasagne", "crespelle", "cannelloni", "nidi", "rosette", "rotolini", "porzioni", "gnocchi"]
       ...
       # ========== GENERA INTESTAZIONE (solo se NON aggiungi servizi) ==========
        ...
        for k, checkbox in self.selected.items():
            if hasattr(checkbox, 'isChecked') and checkbox.isChecked() and self.voce_categoria.get(k) == "cibo":
                ...
                # Dolci (sezione "dolci" O parole chiave dolci nel nome)
                elif "dolci" in k or any(parola_dolce in voce_lower for parola_dolce in [
                    "fagottini crema", "pan au chocolat", "minibrioches dolci",
                    "pasticceria", "biscotteria", "spicchietti crostate",
                    "trancetti tenerina", "crostat", "tiramisu", "cheesecake",
                    "brownie", "macaron", "sfogliat", "cannoncin", "millefoglie",
                    "bigne", "profiter", "tartufi", "praline"
                ]):
                    self.referenze_dolci.append(k)
                    print(f"   ➡️ DOLCI")
        ...
        # 6.5. SPIEDINI FRUTTA (se selezionati)
        self.aggiungi_spiedini_frutta_ai_dolci(cibo_cell, num_pers)
        ...
        def add_inner_table_to_cell(testo_titolo, categoria, parent_cell):
            ...
            for voce in voci_categoria:
                p = inner_cell.add_paragraph()
                run = p.add_run(f"• {voce}")
                
                # Se la voce contiene una delle parole chiave, mettila in grassetto
                if any(speciale in voce.lower() for speciale in voci_speciali):
                    run.bold = True
    ...
    def calcola_quantita_menu_extra_dolci_aperitivo_rinforzato(self, voce, persone, servizio_lower, tipo_ap):
        ...
        return int(persone)
    def copia_voci_selezionate(self):
        """Estrae le voci selezionate, le formatta e le copia nella clipboard di sistema"""
        testo_formattato = "VOCI SELEZIONATE:\n\n"
        voci_trovate = False

        # Cicla tutte le checkbox attualmente selezionate
        for key, checkbox in self.selected.items():
            if checkbox.isChecked():
                voce = self.voce_originale.get(key, "")
                # Usa \t (Tab) per allineare perfettamente il testo quando incollato su Word
                testo_formattato += f"•\t{voce}\n"
                voci_trovate = True

        if voci_trovate:
            # Accede alla clipboard (Appunti) del sistema operativo
            clipboard = QApplication.clipboard()
            clipboard.setText(testo_formattato)
            
            # Mostra un piccolo avviso all'utente
            QMessageBox.information(self, "Successo", "Voci copiate negli appunti!\nOra puoi incollarle su Word.")
        else:
            QMessageBox.warning(self, "Attenzione", "Nessuna voce selezionata da copiare.")
        ...
        for k, checkbox in self.selected.items():
            if hasattr(checkbox, 'isChecked') and checkbox.isChecked():
                if k.startswith("coffee break|"):
                    ...
                    return "Lunch Buffet"
                elif k.startswith("cena servita|"):
                    return "Cena Servita"
...
def estrai_quantita_da_riga_migliorata(self, riga):
    ...
    patterns = [
        ...
        (r'^(\d+)\s+(lasagne|crespelle|cannelloni|rosette|nidi|rotolini|intrighi)(?:\s+(.+))?$', "primi"),
