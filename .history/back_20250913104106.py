...
class GestorePercorsi:
    ...
    def inizializza_percorsi(self):
        ...
        percorsi_possibili = [
            ...
            r"C:\Users\Alex\OneDrive - Alma Mater Studiorum UniversitÃ  di Bologna\Documenti\NeriCatering\dist", # Desktop utente
        ...
        print("ðŸ” Ricerca percorso catering su questo PC...")
        ...
        for percorso in percorsi_possibili:
            if os.path.exists(percorso):
                ...
                if self.verifica_cartella_catering(percorso):
                    ...
                    print(f"âœ… TROVATO percorso catering: {percorso}")
                    break
        
        if not self.percorso_base:
            # Fallback: usa cartella corrente
            self.percorso_base = "./"
            print(f"âš ï¸ Uso cartella corrente come fallback")
...
class CleanLogoSplash(QSplashScreen):
    ...
    def load_logo(self):
        ...
        if not os.path.exists(icon_path):
            print(f"âŒ ERRORE: {icon_path} non trovato nella cartella corrente")
            print(f"ðŸ“ Cartella corrente: {os.getcwd()}")
            print(f"ðŸ“‹ File nella cartella: {os.listdir('.')}")
        ...
        try:
            ...
            if original_pixmap.isNull():
                print(f"âŒ ERRORE: {icon_path} esiste ma non Ã¨ un'immagine valida")
                return self.create_fallback_logo()
            
            print(f"âœ… Icona caricata correttamente: {original_pixmap.width()}x{original_pixmap.height()}")
            ...
            print(f"âœ… Icona ridimensionata: {scaled_pixmap.width()}x{scaled_pixmap.height()}")
            return scaled_pixmap
            
        except Exception as e:
            print(f"âŒ ERRORE caricamento icona: {e}")
            return self.create_fallback_logo()
    
    
    def create_fallback_logo(self):
        """Logo di fallback se icona.png non funziona"""
        print("ðŸ”„ Creazione logo di fallback...")
        ...
        painter.end()
        print("âœ… Logo di fallback creato")
        return pixmap
...
QPushButton:hover {
    background-color: #7c3aed;  /* Viola piÃ¹ scuro al hover */
}

QPushButton:pressed {
    background-color: #6d28d9;  /* Viola ancora piÃ¹ scuro quando premuto */
}
...
QPushButton#primaryBtn:hover {
    background-color: #059669;  /* Verde piÃ¹ scuro al hover */
}
...
QPushButton#secondaryBtn:hover {
    background-color: #dc2626;  /* Rosso piÃ¹ scuro al hover */
}
...
QPushButton#successBtn:hover {
    background-color: #2563eb;  /* Blu piÃ¹ scuro al hover */
}
...
QScrollBar::handle:vertical:hover {
    background-color: #7c3aed;  /* Viola piÃ¹ scuro */
}
...
class BusinessCard(QFrame):
    """Card con stile business (se non esiste giÃ  nel tuo codice)"""
    def __init__(self):
        super().__init__()
...
class StatisticsWindow(QDialog):
    def __init__(self, parent=None):
        ...
        self.setMinimumSize(1000, 700)  # Dimensione minima piÃ¹ grande
        # =====================================================
...
""")
    ...
    def setup_ui(self):
        ...
        self.mese_combo.setMinimumWidth(250)  # PiÃ¹ largo
        self.mese_combo.setMaximumHeight(40)   # PiÃ¹ alto
        filter_layout.addWidget(self.mese_combo)
        
        filter_layout.addStretch()
        
        # Pulsante refresh piÃ¹ grande
        refresh_btn = QPushButton("ðŸ”„ Aggiorna Statistiche")
        ...
        # Ottimizza lo scroll per fluiditÃ 
        self.scroll_area.verticalScrollBar().setSingleStep(20)  # Step piÃ¹ fluido
        ...
        export_btn = QPushButton("ðŸ“Š Esporta Dati")
    ...
    def export_statistics_to_excel(self):
        ...
        try:
            # Controlla se pandas Ã¨ disponibile
            try:
            ...
            for ordine in ordini_filtrati:
                ...
                ordini_data.append({
                    ...
                    "In Fiera": "SÃ¬" if ordine.get("evento_in_fiera", False) else "No",
            ...
            success_msg = f"""ðŸ“Š EXPORT EXCEL COMPLETATO!

    ðŸ“ File: {filename}
    ðŸ“‹ Fogli creati:
    â€¢ Riepilogo (5 metriche principali)
    â€¢ Distribuzione Servizi ({len(distribuzione_data)} tipi)
    â€¢ Ordini Dettagliati ({len(ordini_data)} ordini)

    ðŸ“ˆ Dati esportati:
    â€¢ Periodo: {testo_selezionato}
    â€¢ Ordini: {total_ordini}
    â€¢ Persone: {total_persone}

    ðŸ’¡ Puoi aprire il file con Excel, LibreOffice o Google Sheets"""
            ...
            error_msg = f"âŒ Errore durante l'export:\n\n{str(e)}\n\n"
            
            if "pandas" in str(e) or "openpyxl" in str(e):
                error_msg += "ðŸ’¡ Soluzione: Installa le librerie necessarie:\n"
    ...
    def open_excel_file(self, filepath_completo):
        ...
        try:
            ...
            print(f"ðŸ“„ Excel aperto: {filepath_completo}")
            
        except Exception as e:
            print(f"âŒ Errore apertura Excel: {e}")
    ...
    def crea_riepilogo_generale_migliorato(self, ordini_filtrati, persone_per_servizio):
        ...
        # Header piÃ¹ grande
        summary_header = QLabel("ðŸ“ˆ RIEPILOGO GENERALE")
        ...
        stats_data = [
            ("ðŸ“… Periodo:", periodo_info, "#3498db"),
            ("ðŸ“‹ Ordini Totali:", str(total_ordini), "#2ecc71"),
            ("ðŸ‘¥ Persone Servite:", str(total_persone), "#e74c3c"),
            ("ðŸ“Š Media Persone/Ordine:", f"{media_persone:.1f}", "#f39c12")
        ]
    ...
    def create_chart_improved(self, servizi_count):
        ...
        # Header
        chart_header = QLabel("ðŸ“Š DISTRIBUZIONE SERVIZI")
        ...
        # Dimensioni piÃ¹ grandi per il grafico
        # ========== GRAFICO MATPLOTLIB MIGLIORATO ==========
    ...
    def determina_tipo_servizio(self, ordine):
        ...
        print(f"ðŸ” DEBUG determina_tipo_servizio - Scelte: {[k for k, v in scelte.items() if v]}")
...
class OrderArchiveDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        print(f"ðŸ“‹ Archivio registrato. Totale archivi aperti: {len(archivio_dialogs_aperti)}")
    def ricarica_dati(self):
        """Ricarica i dati dell'archivio dal file JSON - VERSIONE DEBUG"""
        print("ðŸ”„ === INIZIO RICARICAMENTO ARCHIVIO ===")
        ...
        if current_item and current_item.parent() is not None:
            ...
            print(f"ðŸ’¾ Elemento selezionato: {selected_cliente} - {selected_data}")
        
        # Pulisci completamente l'albero
        print("ðŸ§¹ Pulendo albero esistente...")
        self.tree.clear()
        
        # Ricarica dal file
        print("ðŸ“‚ Ricaricando dal file JSON...")
        self.load_orders()
        
        # Prova a ristabilire la selezione
        if selected_cliente and selected_data:
            print(f"ðŸŽ¯ Cercando di ristabilire selezione: {selected_cliente}")
            ...
            for i in range(root.childCount()):
                ...
                for j in range(date_group.childCount()):
                    ...
                    if (order_item.text(1) == selected_cliente and 
                        ...
                        print(f"âœ… Selezione ristabilita")
                        break
        
        print("âœ… === FINE RICARICAMENTO ARCHIVIO ===")
        
        print("âœ… Dati archivio ricaricati")
    def add_selected_services(self):
        ...
        if idx is not None and 0 <= idx < len(self.ordini):
            ...
            reply = QMessageBox.question(
                ...
                f"ðŸ’¡ L'archivio rimarrÃ  aperto per vedere le modifiche",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            
            if reply == QMessageBox.StandardButton.Yes:
                # ========== IMPOSTA MODALITÃ€ AGGIUNGI SERVIZI ==========
                print(f"ðŸ”§ Attivando modalitÃ  aggiungi servizi per ordine in posizione {idx}")
                ...
                if hasattr(self.parent(), 'modalita_aggiungi_servizi'):
                    ...
                    print(f"ðŸ’¡ Memorizzato indice ordine per aggiungi servizi: {idx}")
                    
                    # Debug
                    print(f"ðŸ” Debug ordine per aggiungi servizi:")
                ...
                # AVVISO
                QMessageBox.information(self, "ModalitÃ  Aggiungi Servizi Attivata", 
                                    f"âœ… ModalitÃ  aggiungi servizi attivata!\n\n"
                                    f"ðŸ‘¤ Cliente: {cliente}\n"
                                    f"ðŸ“… Data: {data}\n\n"
                                    f"âž• Seleziona i nuovi servizi da aggiungere\n"
                                    f"ðŸ”„ Premi 'Aggiungi a Ordine Esistente' per salvare\n"
                                    f"ðŸ‘€ L'archivio si aggiornerÃ  automaticamente")
                
                # ========== NON CHIUDERE L'ARCHIVIO! ==========
                # RIMUOVI ENTRAMBE LE RIGHE self.accept()
                print("âœ… ModalitÃ  aggiungi servizi configurata, archivio rimane aperto")
    ...
    def setup_ui(self):
        ...
        # Istruzioni
        help_label = QLabel("ðŸ’¡ Usa Ctrl+Click per selezionare piÃ¹ ordini")
        ...
        load_btn = QPushButton("ðŸ“– Carica Ordine")
        ...
        modify_btn = QPushButton("âœï¸ Modifica Ordine")
        ...
        add_services_btn = QPushButton("âž• Aggiungi Servizi")
        ...
        self.delete_btn = QPushButton("ðŸ—‘ï¸ Elimina Ordine")
    ...
    def update_selection_info(self):
        ...
        if count == 0:
            ...
            self.delete_btn.setText("ðŸ—‘ï¸ Elimina Ordine")
        elif count == 1:
            # Singola selezione
            ...
            # Selezione multipla
            self.selection_info.setText(f"ðŸ”¢ Selezionati {count} ordini")
            self.selection_info.setStyleSheet("color: #dc2626; font-weight: bold; font-size: 14px;")
            self.delete_btn.setEnabled(True)
            self.delete_btn.setText(f"ðŸ—‘ï¸ Elimina {count} Ordini")
    ...
    def load_orders(self):
        ...
        if hasattr(self.parent(), '_ordine_appena_modificato') and self.parent()._ordine_appena_modificato:
            print("ðŸ”„ Rilevate modifiche recenti, ricaricando archivio...")
    ...
    def populate_tree(self, filter_text=""):
        ...
        # ========== ORDINAMENTO: DAL PIÃ™ LONTANO IN ALTO ==========
        # reverse=False mette le date piÃ¹ LONTANE PRIMA (in alto)
        date_ordinate = sorted(ordini_per_data.keys(), 
                            key=get_datetime, 
                            reverse=False)  # âœ… CAMBIATO: False = ordine cronologico normale

        print(f"ðŸ” DEBUG ORDINAMENTO CRONOLOGICO:")
        for data in date_ordinate[:5]:  # Mostra prime 5 date
            try:
                data_obj = datetime.strptime(data, "%d/%m/%Y")
                print(f"   ðŸ“… {data} ({data_obj.strftime('%A')})")
            except:
                print(f"   ðŸ“… {data}")
        ...
        for data in date_ordinate:
            ...
            try:
                ...
                header_text = f"ðŸ“… {data_formattata}"
            except (ValueError, TypeError):
                header_text = f"ðŸ“… {data}"
            ...
            # ========== SEMPRE CHIUSO ==========
            parent.setExpanded(False)  # âœ… SEMPRE CHIUSO
    ...
    def modify_selected_order(self):
        ...
        if idx is not None and 0 <= idx < len(self.ordini):
            ...
            reply = QMessageBox.question(
                ...
                f"Cliente: {cliente}\n"
                f"Data: {data}\n\n"
                f"âš ï¸ Il documento Word esistente verrÃ  SOVRASCRITTO\n"
                f"ðŸ’¡ L'archivio rimarrÃ  aperto per vedere le modifiche",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            
            if reply == QMessageBox.StandardButton.Yes:
                # ========== IMPOSTA MODALITÃ€ MODIFICA E MEMORIZZA INDICE ==========
                if hasattr(self.parent(), 'modalita_modifica'):
                    ...
                    self.parent().indice_ordine_originale = idx
                    print(f"ðŸ’¡ Memorizzato indice ordine originale: {idx}")
                    
                    # Debug aggiuntivo
                    print(f"ðŸ” Debug ordine da modificare:")
                    print(f"   - Nome: {cliente}")
                    print(f"   - Data: {data}")
                    print(f"   - Path: {ordine.get('path_docx', 'N/A')}")
                    print(f"   - Indice: {idx}")
                ...
                # AVVISO
                QMessageBox.information(self, "ModalitÃ  Modifica Attivata", 
                                    f"âœ… Ordine caricato per modifica!\n\n"
                                    f"ðŸ‘¤ Cliente: {cliente}\n"
                                    f"ðŸ“… Data: {data}\n\n"
                                    f"ðŸ”§ Fai le tue modifiche nella finestra principale\n"
                                    f"ðŸ“„ Premi 'Genera ordine' per salvare\n"
                                    f"ðŸ‘€ L'archivio si aggiornerÃ  automaticamente")
    ...
    def delete_selected_orders(self):
        ...
        if not selected_orders:
            QMessageBox.information(self, "Selezione", "Seleziona uno o piÃ¹ ordini da eliminare.")
            return
        ...
        if count == 1:
            ...
            for item in selected_orders[:5]:  # Mostra max 5 nomi
                ...
                if idx is not None and idx < len(self.ordini):
                    ...
                    data = ordine["campi"].get("Data evento", "")
                    clienti_list.append(f"â€¢ {cliente} ({data})")
        ...
        reply = QMessageBox.question(
            self, f"âš ï¸ CONFERMA ELIMINAZIONE {count} ORDINI" if count > 1 else "âš ï¸ CONFERMA ELIMINAZIONE", 
            f"{messaggio}\n\n"
            f"âš ï¸ ATTENZIONE:\n"
            f"â€¢ Gli ordini verranno rimossi dall'archivio\n"
            f"â€¢ I file Word verranno spostati nel cestino\n"
            f"â€¢ Questa operazione NON puÃ² essere annullata\n\n"
            f"Vuoi procedere?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    ...
    def esegui_eliminazione_multipla(self, selected_orders):
        ...
        # Raccogli indici da eliminare (dal piÃ¹ alto al piÃ¹ basso per evitare shift)
        indici_da_eliminare = []
        ...
        indici_da_eliminare.sort(reverse=True)  # Dal piÃ¹ alto al piÃ¹ basso
        ...
        try:
            ...
            for i, idx in enumerate(indici_da_eliminare):
                try:
                    ...
                    errori.append(f"â€¢ {cliente}: {str(e)}")
            ...
            # Messaggio risultato
            messaggio = f"âœ… Eliminazione completata!\n\n" \
                    f"ðŸ“‹ Ordini eliminati: {eliminati}/{count}\n" \
                    f"ðŸ“„ File spostati nel cestino: {files_spostati}"
            
            if errori:
                messaggio += f"\n\nâš ï¸ Errori durante l'eliminazione:\n" + "\n".join(errori)
            ...
            QMessageBox.critical(
                ...
                f"âŒ Errore durante l'eliminazione multipla:\n\n{str(e)}"
            )
    ...
    def closeEvent(self, event):
        ...
        if self in archivio_dialogs_aperti:
            ...
            print(f"ðŸ“‹ Archivio rimosso. Totale archivi aperti: {len(archivio_dialogs_aperti)}")
        event.accept()
...
class CustomCheckBox(QCheckBox):
    ...
    def paintEvent(self, event):
        ...
        if self.isChecked():
            ...
            # Se il rect non Ã¨ valido, usa calcolo manuale migliorato
            ...
            offset = 5  # Aumento l'offset per una X piÃ¹ grande
...
class EventBadge(QFrame):
    ...
    def setup_ui(self):
        ...
        persone_label = QLabel(f"ðŸ‘¥{persone}")
    ...
    def setup_style(self):
        ...
        # IntensitÃ  basata su numero persone (piÃ¹ persone = piÃ¹ scuro)
        if persone <= 20:
            opacity = "0.7"
        elif persone <= 50:
            opacity = "0.85"
    ...
    def setup_tooltip(self):
        ...
        tooltip_text = f"""
<b>ðŸ·ï¸ {campi.get('Nome', 'Cliente')}</b><br>
ðŸ“… {campi.get('Data evento', '')}<br>
ðŸ‘¥ {campi.get('Numero persone', '0')} persone<br>
ðŸ½ï¸ {servizio}<br>
ðŸ“ž {campi.get('Cellulare', 'N/A')}<br>
ðŸ“ {campi.get('Via', 'N/A')}<br>
â° {campi.get('Orario allestimento', 'N/A')} - {campi.get('Orario Pronti', 'N/A')}
        """.strip()
        
        if self.ordine_data.get("evento_in_fiera", False):
            tooltip_text += "<br>ðŸ¢ <b>EVENTO IN FIERA</b>"
    ...
    def determina_servizio(self):
        ...
        if any(k.startswith("coffee break|") and v for k, v in scelte.items()):
            ...
            return "Lunch box"
        elif any(k.startswith("servito|") and v for k, v in scelte.items()):  # â† NUOVO
            return "Servito"
...
class DayCell(QFrame):
    ...
    def __init__(self, day_number, ordini_giorno=None, is_current_month=True):
        ...
        # Imposta proprietÃ  per giorno corrente
...
class CalendarioEventiProfessionale(QWidget):
    ...
    def create_professional_header(self, parent_layout):
        ...
        # Pulsanti eleganti
        self.prev_btn = self.create_nav_button("â®", self.prev_month)
        self.next_btn = self.create_nav_button("â¯", self.next_month)
    ...
    def create_view_controls(self, parent_layout):
        ...
        month_btn = QPushButton("ðŸ“… Mese")
        week_btn = QPushButton("ðŸ“‹ Settimana")
...
class EventBadge(QFrame):
    ...
    def setup_premium_style(self):
        ...
        # OpacitÃ  basata su numero persone
        if persone <= 20:
            opacity = "0.85"
        elif persone <= 50:
            opacity = "0.95"
    ...
    def setup_premium_tooltip(self):
        ...
        ">
            <h3 style="
                ...
                font-size: 14px;
                font-weight: 600;
            ">ðŸ·ï¸ {campi.get('Nome', 'Cliente')}</h3>
            
            <div style="color: #6b7280; font-size: 12px; line-height: 1.4;">
                ðŸ“… <strong>{campi.get('Data evento', '')}</strong><br>
                ðŸ‘¥ {campi.get('Numero persone', '0')} persone<br>
                ðŸ½ï¸ {servizio}<br>
                ðŸ“ž {campi.get('Cellulare', 'N/A')}<br>
                ðŸ“ {campi.get('Via', 'N/A')}<br>
...
class CalendarioEventi(QWidget):
    ...
    def setup_premium_ui(self):
        ...
        self.prev_btn = QPushButton("â—€ Prec")
        ...
        self.next_btn = QPushButton("Succ â–¶")
    ...
    def load_ordini(self):
        ...
        try:
            ...
            print(f"âŒ Errore caricamento ordini: {e}")
            self.ordini_data = []
    ...
    def refresh_calendar(self):
        ...
        # Giorno della settimana del primo giorno (0=lunedÃ¬)
    ...
    def show_day_options(self, day_number, ordini_giorno):
        ...
        # Header
        header = QLabel(f"ðŸ“… {day_number} {self.current_date.strftime('%B %Y')}")
        header.setAlignment(Qt.AlignmentFlag.AlignCenter)
        ...
        info_text = f"""
        ðŸ“‹ <b>{len(ordini_giorno)} ordini programmati</b><br>
        ðŸ‘¥ <b>{total_persone} persone totali</b><br><br>
        ...
        for servizio, count in servizi_count.items():
            info_text += f"â€¢ {servizio}: {count} ordini<br>"
        ...
        view_btn = QPushButton("ðŸ‘ï¸ Visualizza Dettagli Ordini")
        ...
        # Pulsante Export Tabella Operativa - QUESTO Ãˆ IL PULSANTE PRINCIPALE!
        export_btn = QPushButton("ðŸ“‹ Genera Tabella Operativa")
    ...
    def show_day_details_table(self, day_number, ordini_giorno, parent_dialog):
        ...
        # Header
        header = QLabel(f"ðŸ“… {len(ordini_giorno)} ordini programmati")
        header.setFont(QFont("Arial", 16, QFont.Weight.Bold))
        header.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def export_day_to_excel(self, day_number, ordini_giorno):
        ...
        try:
            ...
            success_msg = f"""ðŸ“‹ TABELLA OPERATIVA CREATA!
    ...
    def generate_kitchen_summary(self, day_number, ordini_giorno):
        ...
        try:
            ...
            if hasattr(self.parent(), 'crea_riepilogo_cucina'):
                ...
                QMessageBox.information(self, "FunzionalitÃ ", 
...
class CalendarioEventiDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("ðŸ“… Calendario Eventi Catering")
        ...
        header = QLabel("ðŸ“… CALENDARIO EVENTI")
        header.setStyleSheet("""
            QLabel {
                font-size: 24px;
                font-weight: 700;
        ...
        # Info badge
        info_label = QLabel("ðŸ’¡ Clicca su un giorno per gestire gli ordini")
...
class CateringApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Sostituiscila con:
        self.setGeometry(100, 100, 1400, 1300)  # larghezza=1400, altezza=1200 (piÃ¹ lungo!)  # Finestra piÃ¹ grande
        ...
        rinomina_btn = QPushButton("ðŸ”„ Rinomina Referenza")
        ...
        try:
            with open(gestore_percorsi.get_percorso("referenze_cibi.json"), "r", encoding="utf-8") as f:
                ...
                print("ðŸ” DEBUG CARICAMENTO JSON:")
                for k, v in raw_data.items():
                    if "verdi" in k.lower():
                        print(f"   JSON raw: '{k}' â†’ {v}")
                        print(f"   Convertito: '{k.strip().lower()}' â†’ {v}")
        except FileNotFoundError:
            self.dati_referenze = {}
            print("Warning: referenze_cibi.json non trovato. VerrÃ  utilizzato il calcolo base.")
    ...
    def setup_ui(self):
        ...
        title = QLabel("ð“ð“®ð“»ð“² ð“–ð“®ð“¼ð“½ð“²ð“¸ð“·ð“® ð“’ð“ªð“½ð“®ð“»ð“²ð“·ð“°")
    ...
    def create_menu(self):
        ...
        new_action = QAction("ðŸ†•Nuovo Ordine", self)
        new_action.triggered.connect(self.nuovo_ordine)
        file_menu.addAction(new_action)
        
        file_menu.addSeparator()
        
        exit_action = QAction("âŽEsci", self)
        ...
        calendario_action = QAction("ðŸ“… Visualizza Calendario", self)
        ...
        stats_action = QAction("ðŸ“ŠVisualizza Statistiche", self)
        stats_action.triggered.connect(self.mostra_statistiche)
        stats_menu.addAction(stats_action)
        
        riepilogo_action = QAction("ðŸ§‘ðŸ»â€ðŸ³Riepilogo Cucina", self)
        ...
        rinomina_action = QAction("ðŸ”„ Rinomina Referenza", self)
        ...
        about_action = QAction("ðŸ‘¨ðŸ»â€ðŸ’»Informazioni", self)
    ...
    def setup_search_functionality(self):
        """Configura la funzionalitÃ  di ricerca"""
    ...
    def open_search(self):
        """Apre la barra di ricerca e le dÃ  il focus"""
    ...
    def filter_referenze(self, text):
        ...
        # Se il testo Ã¨ vuoto, mostra tutto subito
        if not text.strip():
            self.show_all_referenze()
    ...
    def perform_search(self):
        ...
        for tab_index in range(self.tabs.count()):
            ...
            for group_box in group_boxes:
                ...
                for checkbox in checkboxes:
                    ...
                    # Verifica se il testo di ricerca Ã¨ presente
        ...
        if visible_count == 0:
            self.search_bar.results_label.setText("ðŸš« Nessun risultato trovato")
            self.search_bar.results_label.setStyleSheet("color: #dc2626; font-size: 12px;")
        else:
            self.search_bar.results_label.setText(f"âœ… {visible_count} di {total_count} referenze")
    ...
    def create_order_info_section(self, parent_layout):
        ...
        self.info_header = QPushButton("ðŸ”¼ INFORMAZIONI CLIENTE ðŸ‘¥ ")
       ...
       # Checkbox "In Fiera" - usa CustomCheckBox per X blu
        self.fiera_checkbox = CustomCheckBox("ðŸ¢ Evento in Fiera")
        ...
        self.aggiungi_ordine_btn = QPushButton("âž• Aggiorna l'ordine")
    ...
    def rinomina_referenza_globale(self, vecchio_nome, nuovo_nome):
        ...
        try:
                ...
                if isinstance(obj, list):
                    for i, item in enumerate(obj):
                        if isinstance(item, str) and item.strip() == vecchio_nome:
                            obj[i] = nuovo_nome
                            modificato = True
                            print(f"ðŸ”„ Sostituito in lista: '{vecchio_nome}' â†’ '{nuovo_nome}'")
            ...
            if vecchio_nome in referenze_data:
                ...
                print(f"ðŸ“‹ Dati vecchia referenza: {dati_vecchia_referenza}")
                
                # PRIMA: Aggiungi la nuova referenza con gli stessi dati
                referenze_data[nuovo_nome] = dati_vecchia_referenza
                print(f"âœ… Aggiunta nuova referenza: '{nuovo_nome}'")
                
                # POI: Rimuovi la vecchia referenza
                del referenze_data[vecchio_nome]
                print(f"ðŸ—‘ï¸ Rimossa vecchia referenza: '{vecchio_nome}'")
                
                modificato_referenze = True
            else:
                print(f"âš ï¸ Referenza '{vecchio_nome}' non trovata in referenze_cibi.json")
                ...
                modificato_referenze = True
                print(f"âž• Aggiunta nuova referenza con valori default: '{nuovo_nome}'")
            ...
            if modificato_catering:
                ...
                messaggi_log.append("âœ… catering_data.json aggiornato")
            else:
                messaggi_log.append("âš ï¸ Nessuna modifica in catering_data.json")
            ...
            if modificato_referenze:
                ...
                messaggi_log.append("âœ… referenze_cibi.json aggiornato")
            else:
                messaggi_log.append("âš ï¸ Nessuna modifica in referenze_cibi.json")
            
            # 7. Aggiorna anche i dati in memoria
            self.ricarica_dati_referenze()
            messaggi_log.append("ðŸ”„ Dati in memoria ricaricati")
            
            # 8. Verifica finale
            print("\n" + "="*50)
            print("ðŸ“Š VERIFICA FINALE:")
            print("="*50)
            ...
            print(f"ðŸ” Vecchia referenza '{vecchio_nome}': {'âŒ ANCORA PRESENTE!' if vecchia_presente else 'âœ… Rimossa'}")
            print(f"ðŸ” Nuova referenza '{nuovo_nome}': {'âœ… Presente' if nuova_presente else 'âŒ MANCANTE!'}")
            
            # Messaggio finale
            messaggio_successo = (
                f"ðŸŽ‰ RINOMINAZIONE COMPLETATA!\n\n"
                f"Da: {vecchio_nome}\n"
                f"A: {nuovo_nome}\n\n"
                f"ðŸ“‹ DETTAGLI:\n" + "\n".join(messaggi_log) + "\n\n"
                f"ðŸ’¾ BACKUP CREATI:\n"
                ...
                f"ðŸ” VERIFICA:\n"
                f"- Vecchia referenza: {'âŒ Ancora presente!' if vecchia_presente else 'âœ… Rimossa'}\n"
                f"- Nuova referenza: {'âœ… Presente' if nuova_presente else 'âŒ Mancante!'}"
            )
    ...
    def dialog_rinomina_referenza(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("ðŸ”„ Rinomina Referenza Globale")
        ...
        # Label che mostra cosa verrÃ  cercato
        ...
        def aggiorna_anteprima():
            ...
            if vecchio_pulito:
                vecchio_pulito_label.setText(f"ðŸ” CercherÃ²: {vecchio_pulito}")
            else:
                vecchio_pulito_label.setText("")
                
            if nuovo_pulito:
                nuovo_pulito_label.setText(f"âœ¨ DiventerÃ : {nuovo_pulito}")
            else:
                nuovo_pulito_label.setText("")
            
            # Anteprima finale
            if vecchio_pulito and nuovo_pulito:
                anteprima_label.setText(f"ðŸ”„ OPERAZIONE: '{vecchio_pulito}' â†’ '{nuovo_pulito}'")
        ...
        test_btn = QPushButton("ðŸ” Testa se Esiste")
        ...
        def testa_esistenza():
            ...
            try:
                ...
                # Risultato
                msg = f"ðŸ” RISULTATO TEST per: '{vecchio_pulito}'\n\n"
                msg += f"ðŸ“„ catering_data.json: {'âœ… TROVATO' if trovato_catering else 'âŒ NON trovato'}\n"
                msg += f"ðŸ“Š referenze_cibi.json: {'âœ… TROVATO' if trovato_referenze else 'âŒ NON trovato'}\n\n"
                
                if trovato_catering or trovato_referenze:
                    msg += "âœ… Puoi procedere con la rinominazione!"
                else:
                    msg += "âš ï¸ Nome non trovato in nessun file. Controlla l'ortografia."
        ...
        rinomina_btn = QPushButton("ðŸ”„ RINOMINA")
        rinomina_btn.setStyleSheet("""
            QPushButton {
                background: #dc2626;
                color: white;
        ...
        def esegui_rinomina():
            ...
            # âœ… CORREZIONE QUI - Sintassi PyQt6
            reply = QMessageBox.question(dialog, "Conferma Rinominazione", 
                f"ðŸ”„ CONFERMI LA RINOMINAZIONE?\n\n"
                f"Da: {vecchio_pulito}\n"
                f"A:  {nuovo_pulito}\n\n"
                f"âš ï¸ Questa operazione modificherÃ  entrambi i file JSON!\n"
                f"ðŸ’¾ Verranno creati backup automatici.",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def ricarica_dati(self):
        ...
        print("ðŸ”„ Ricaricando dati archivio...")
        self.load_orders()
        print("âœ… Dati archivio ricaricati")
    ...
    def toggle_compact_mode(self):
            """Alterna tra modalitÃ  normale e compatta"""
            if hasattr(self, 'compact_mode') and self.compact_mode:
                # Torna alla modalitÃ  normale
                self.info_card.setMaximumHeight(16777215)  # Rimuovi limite altezza
                self.tabs.setMinimumHeight(500)
                self.compact_mode = False
                self.compact_btn.setText("ðŸ“‹ ModalitÃ  Compatta")
            else:
                # Attiva modalitÃ  compatta
                self.info_card.setMaximumHeight(200)  # Comprimi info ordine
                self.tabs.setMinimumHeight(700)  # PiÃ¹ spazio per i menu
                self.compact_mode = True
                self.compact_btn.setText("ðŸ“‹ ModalitÃ  Normale")
    
    def set_evento_fiera(self, checked):
        """Imposta se l'evento Ã¨ in fiera"""
        self.evento_in_fiera = checked
        if checked:
            print("âœ… Evento IN FIERA - 200g per persona")
        else:
            print("ðŸ“ Evento STANDARD - calcolo normale")
    ...
    def toggle_info_section(self):
        ...
        if self.info_expanded:
            ...
            self.info_header.setText("ðŸ”½ INFORMAZIONI CLIENTE ðŸ‘¥")
            self.info_expanded = False
            
            # ========== AGGIUNGI QUESTE RIGHE ==========
            # PiÃ¹ spazio per i tabs quando compressi
            screen_height = QApplication.primaryScreen().geometry().height()
    ...
    def resizeEvent(self, event):
        ...
        if hasattr(self, 'tabs'):
            if hasattr(self, 'info_expanded') and not self.info_expanded:
                # Se info Ã¨ compressa, usa piÃ¹ spazio
                new_height = int(self.height() * 0.65)
            else:
                # Se info Ã¨ espansa, usa spazio normale
    ...
    def add_service_tab(self, name, sections):
        ...
        # Se Ã¨ aperitivo a buffet, aggiungi controlli tipo
        if name.lower() == "aperitivo a buffet":
            ...
            def on_leggero_clicked():
                if leggero_radio.isChecked():
                    ...
                    print("âœ… Selezionato: Aperitivo Leggero")
                else:
                    # Se si deseleziona leggero, azzera il tipo
                    self.set_ap_tipo("")
                    print("âŒ Deselezionato: Aperitivo Leggero")

            def on_rinforzato_clicked():
                if rinforzato_radio.isChecked():
                    leggero_radio.setChecked(False)
                    self.set_ap_tipo("Rinforzato")
                    print("âœ… Selezionato: Aperitivo Rinforzato")
                else:
                    # Se si deseleziona rinforzato, azzera il tipo
                    self.set_ap_tipo("")
                    print("âŒ Deselezionato: Aperitivo Rinforzato")
    ...
    def contains_food_items(self, items):
        ...
        food_keywords = [
            ...
            "pasta", "riso", "lasagne", "gnocchi", "risotto", "dolc", "tiramisÃ¹",
    ...
    def are_real_accessories(self, items):
        ...
        # Debug degli items
        print(f"ðŸ” Analizzando {len(items)} items per accessori:")
        ...
        if "crostini" in item_lower or "rombetti" in item_lower or "bocconcini" in item_lower:
            print(f"ðŸ” ANALISI SPECIFICA: '{item}'")
            print(f"  â†’ Parole trovate: {[kw for kw in accessory_keywords if kw in item_lower]}")
            print(f"  â†’ Categorizzato come: {'ACCESSORIO' if is_accessory else 'CIBO'}")
            
            if is_accessory:
                accessory_count += 1
                print(f"  âœ… ACCESSORIO: {item}")
            else:
                print(f"  ðŸ• CIBO: {item}")
        
        percentage = (accessory_count / len(items)) * 100 if len(items) > 0 else 0
        print(f"ðŸ“Š Accessori: {accessory_count}/{len(items)} ({percentage:.1f}%)")
        
        # Se piÃ¹ del 30% sono accessori, considera la sezione come accessori
        is_accessory_section = percentage > 30
        print(f"ðŸ·ï¸ Risultato: {'ACCESSORI' if is_accessory_section else 'CIBO'}")
    ...
    def add_service_section(self, parent_layout, tab_name, section_name, items):
        ...
        if any(keyword in section_name_lower for keyword in ["bevande", "bibite", "acqua", "prosecco", "aperol", "caffÃ¨", "tÃ©"]):
            section_type = "bevande"
            print(f"ðŸ¥¤ Categorizzata come: BEVANDE")
        elif any(keyword in section_name_lower for keyword in ["accessori", "sala", "attrezzature", "servizio"]):
            section_type = "accessori"
            print(f"ðŸ› ï¸ Categorizzata come: ACCESSORI")
        elif self.are_real_accessories(items):
            section_type = "accessori"
            print(f"ðŸ› ï¸ Categorizzata come: ACCESSORI (by content)")
        else:
            section_type = "cibo"
        ...
        if any(keyword in section_name_lower for keyword in food_sections):
            section_type = "cibo"
            print(f"ðŸ• FORZATO COME CIBO (whitelist): {section_name}")
            ...
            section_type = "cibo"
            print(f"ðŸ• Categorizzata come: CIBO")
        ...
        for i, item in enumerate(items):
            ...
            # NUOVO: Se Ã¨ un primo, collega la funzione di auto-selezione
            ...
            # NUOVO: Se Ã¨ una bevanda con CONS, collega la funzione di auto-selezione
            ...
            # CORREZIONE: Collega le insalate all'evento toggled invece di chiamare subito
            if any(insalata in item.lower() for insalata in [
            ...
            # NUOVO: Se Ã¨ un secondo piatto o contorno, collega la funzione di auto-selezione
            ...
            if section_type == "accessori":
                print(f"  âœ… Registrato accessorio: {item} â†’ {key[:50]}...")
    ...
    def set_ap_tipo(self, tipo):
        ...
        print(f"ðŸ”§ Tipo aperitivo impostato: '{tipo}'")
    ...
    def create_menu_extra_section_special(self, parent_layout, section_name, items):
        ...
        # Titolo
        title_label = QLabel(f"ðŸŒ± {section_name.upper()}")
        ...
        def seleziona_tutto_menu():
            ...
            print(f"âœ… Selezionate tutte le voci del menu {menu_type}")
        ...
        def deseleziona_tutto_menu():
            ...
            spinbox.setValue(0)
            print(f"âŒ Deselezionate tutte le voci del menu {menu_type} e azzerato numero persone")
        ...
        # Lista delle voci (come sezione normale ma piÃ¹ compatta)
        ...
        # Nota informativa
        info_label = QLabel("ðŸ’¡ Le quantitÃ  verranno calcolate automaticamente per il numero di persone specificato")
    ...
    def aggiungi_riquadro_note(self, doc):
        ...
        run_title.bold = True
        run_title.font.size = Pt(10)  # Font piÃ¹ piccolo
    ...
    def deseleziona_tutto(self):
        ...
        if hasattr(self, 'menu_extra_widgets'):
            for menu_type, widget in self.menu_extra_widgets.items():
                widget.setValue(0)
                print(f"   ðŸ”„ Azzerato widget persone per {menu_type}")
        
        print("âŒ Tutte le selezioni sono state annullate e tutti i contatori persone azzerati")

    
    def nuovo_ordine(self):
        """Crea un nuovo ordine pulito"""
        
        # ========== RESET MODALITÃ€ AGGIUNGI SERVIZI ==========
        ...
        print("âœ… Nuovo ordine - tutte le modalitÃ  resettate")
    ...
    def genera_riepilogo_cucina(self):
        ...
        # Descrizione
        desc = QLabel("Seleziona la data per generare il riepilogo dettagliato delle quantitÃ  necessarie in cucina.")
    ...
    def crea_riepilogo_cucina(self, data_target):
        ...
        # Raggruppa tutte le voci di cibo con le relative quantitÃ 
        ...
        for i, ordine in enumerate(ordini_giorno, 1):
            ...
            print(f"\nðŸ“‹ === ORDINE {i}: {nome_cliente} ({persone} persone) ===")
            ...
            if path_docx and os.path.exists(path_docx):
                ...
                if quantita_da_word:
                    print(f"âœ… Usato quantitÃ  modificate dal Word: {len(quantita_da_word)} voci")
                else:
                    print("âš ï¸  Nessuna modifica trovata nel Word, uso calcolo automatico")
            else:
                print("âŒ File Word non trovato, uso calcolo automatico")
            ...
            print(f"ðŸ¢ Fiera: {ordine_in_fiera}")
            ...
            # ========== CALCOLA O USA QUANTITÃ€ DAL WORD (VERSIONE CORRETTA) ==========
            if quantita_da_word:
                # USA TUTTE LE QUANTITÃ€ DAL WORD
                print("ðŸ“„ Usando TUTTE le quantitÃ  dal Word:")
                for key, info in quantita_da_word.items():
                    voce = info['nome']
                    quantita = info['quantita']
                    print(f"   ðŸ“ {voce}: {quantita} (dal Word)")
                    self.aggrega_quantita_riepilogo(riepilogo_cibo, voce, quantita)
            else:
                # CALCOLO AUTOMATICO NORMALE
                print("ðŸ§® Calcolando automaticamente:")
                for key in referenze_cibo:
                    ...
                    print(f"   ðŸ§® {voce}: {quantita} (calcolato)")
                    self.aggrega_quantita_riepilogo(riepilogo_cibo, voce, quantita)
        ...
        if riepilogo_cibo:
            ...
            header_run = header_p.add_run("RIEPILOGO QUANTITÃ€ TOTALI")
            ...
            def ordina_cibo(item):
                ...
                if any(x in voce for x in ["lasagne", "crespelle", "cannelloni", "rosette", "nidi", "gnocchi"]):
                    return (1, voce)
                elif "dolc" in voce or "torta" in voce or "crostat" in voce or "tiramisÃ¹" in voce:
                    return (3, voce)
                else:
                    return (2, voce)
...
# SOLUZIONE: Migliorare il metodo leggi_quantita_da_word per leggere TUTTO
    def aggrega_quantita_riepilogo(self, riepilogo_cibo, voce, quantita):
        """Aggrega quantitÃ  nel riepilogo cucina RISPETTANDO l'unitÃ  originale"""
        
        # ========== RISPETTA L'UNITÃ€ ORIGINALE DAL WORD ==========
        if isinstance(quantita, str):
            if "kg" in quantita.lower():
                # MANTIENI KG se la quantitÃ  originale Ã¨ in kg
                try:
                    peso = float(quantita.split()[0])
                    riepilogo_cibo[voce]["quantita"] += peso
                    riepilogo_cibo[voce]["unita"] = "kg"
                    print(f"   âœ… Aggregato {voce}: +{peso} kg (totale: {riepilogo_cibo[voce]['quantita']} kg)")
                except (ValueError, IndexError):
                    riepilogo_cibo[voce]["quantita"] += 1
                    riepilogo_cibo[voce]["unita"] = "kg"
            
            elif any(x in quantita.lower() for x in ["lasagne", "crespelle", "cannelloni", "rosette", "nidi"]):
                # Primi elaborati: mantieni unitÃ  specifica
                try:
                    ...
                    print(f"   âœ… Aggregato {voce}: +{numero} {tipo_porzione}")
                except (ValueError, IndexError):
                    riepilogo_cibo[voce]["quantita"] += 1
                    riepilogo_cibo[voce]["unita"] = "porzioni"
            
            elif "pz" in quantita.lower():
                # MANTIENI PZ se la quantitÃ  originale Ã¨ in pezzi
                try:
                    numero = int(quantita.split()[0])
                    riepilogo_cibo[voce]["quantita"] += numero
                    riepilogo_cibo[voce]["unita"] = "pz"
                    print(f"   âœ… Aggregato {voce}: +{numero} pz (totale: {riepilogo_cibo[voce]['quantita']} pz)")
                except (ValueError, IndexError):
                    riepilogo_cibo[voce]["quantita"] += 1
                    riepilogo_cibo[voce]["unita"] = "pz"
            
            else:
                # Numero senza unitÃ : controlla se Ã¨ lasagne
                try:
                    numero = int(quantita)
                    ...
                    if "lasagne" in voce.lower():
                        ...
                        print(f"   âœ… Aggregato {voce}: +{numero} teglie (totale: {riepilogo_cibo[voce]['quantita']} teglie)")
                    else:
                        riepilogo_cibo[voce]["quantita"] += numero
                        riepilogo_cibo[voce]["unita"] = "pz"
                        print(f"   âœ… Aggregato {voce}: +{numero} pz (totale: {riepilogo_cibo[voce]['quantita']} pz)")
            ...
            # QuantitÃ  numerica: controlla se Ã¨ lasagne
            try:
                numero = int(quantita)
    ...
    def formatta_quantita_riepilogo(self, quantita, unita, voce):
        """Formatta la quantitÃ  per il riepilogo cucina SENZA duplicazioni"""
    ...
    def leggi_quantita_da_word(self, path_docx):
        """Legge quantitÃ  da Word - VERSIONE MIGLIORATA per tutti i formati"""
        if not os.path.exists(path_docx):
            print(f"âŒ File Word non trovato: {path_docx}")
            return {}
        ...
        try:
            ...
            print(f"\nðŸ“„ === LETTURA COMPLETA DA: {os.path.basename(path_docx)} ===")
            ...
            print(f"ðŸ“‹ Analizzando {len(righe)} righe totali...")
            
            for riga in righe:
                prodotto_trovato = False
                
                # ========== PATTERN 1: PESO IN KG (PRIORITÃ€ ALTA) ==========
                import re
                    ...
                    if match:
                        ...
                        if nome in prodotti:
                            ...
                            if "kg" in str(prodotti[nome]):
                                ...
                                print(f"   ðŸ”„ {nome}: {peso_esistente} + {peso} = {peso_esistente + peso} kg")
                            else:
                                prodotti[nome] = f"{peso} kg"
                                print(f"   âœ… {nome}: {peso} kg (nuovo)")
                        else:
                            prodotti[nome] = f"{peso} kg"
                            print(f"   âœ… {nome}: {peso} kg (nuovo)")
                ...
                for i, pattern in enumerate(patterns_num):
                    ...
                    if match:
                        ...
                        skip_keywords = [
                            ...
                            "stand", "lunedÃ¬", "martedÃ¬", "mercoledÃ¬", "giovedÃ¬", 
                            "venerdÃ¬", "sabato", "domenica", "gennaio", "febbraio", 
                        ...
                        if any(keyword == nome_lower or nome_lower.startswith(keyword + " ") for keyword in skip_keywords):
                            print(f"   â­ï¸ SALTATO (keyword): {nome}")
                            continue
                        
                        # Salta se Ã¨ solo un numero o troppo corto
                        if len(nome.split()) == 0 or nome.isdigit() or len(nome) < 3:
                            print(f"   â­ï¸ SALTATO (troppo corto): {nome}")
                            continue
                        
                        # ========== ACCUMULA QUANTITÃ€ ==========
                        if nome in prodotti:
                            # Prodotto giÃ  esistente, somma le quantitÃ 
                            try:
                                if "kg" not in str(prodotti[nome]):
                                    ...
                                    print(f"   ðŸ”„ {nome}: {quantita_esistente} + {quantita} = {nuova_quantita}")
                                else:
                                    print(f"   âš ï¸ Saltato accumulo kg/pezzi per: {nome}")
                            except:
                                prodotti[nome] = quantita_str
                                print(f"   âš ï¸ Errore accumulo, impostato: {nome} = {quantita}")
                        else:
                            # Nuovo prodotto
                            prodotti[nome] = quantita_str
                            print(f"   âœ… {nome}: {quantita_str} (nuovo)")
                ...
                if not prodotto_trovato:
                    ...
                    if len(riga) > 5 and not any(skip in riga.lower() for skip in ["bevande", "sala", "nr progr"]):
                        print(f"   â“ NON RICONOSCIUTO: '{riga}'")
            ...
            print(f"\nðŸ“Š === RIEPILOGO FINALE MIGLIORATO ===")
            print(f"âœ… Trovati {len(risultato)} prodotti:")
            for key, info in risultato.items():
                print(f"   ðŸ“ {info['nome']}: {info['quantita']}")
            
            return risultato
            
        except Exception as e:
            print(f"âŒ Errore lettura Word: {e}")
            import traceback
            traceback.print_exc()
            return {}
    ...
    def trova_chiave_corrispondente_migliorata(self, nome_prodotto):
        ...
        print(f"   ðŸ” Cercando chiave per: '{nome_prodotto}' (lower: '{nome_lower}')")
        ...
        for pattern_nome, parole_chiave in match_specifici.items():
            if all(parola in nome_lower for parola in parole_chiave):
                print(f"   ðŸŽ¯ Pattern specifico riconosciuto: {pattern_nome}")
                ...
                for key, voce_originale in self.voce_originale.items():
                    ...
                    if all(parola in voce_lower for parola in parole_chiave):
                        print(f"   âœ… MATCH SPECIFICO: {key} â†’ '{voce_originale}'")
                        return key
        ...
        for key, voce_originale in self.voce_originale.items():
            ...
            if nome_lower == voce_lower:
                print(f"   âœ… MATCH DIRETTO: {key}")
                return key
        
        # 3. Match parziale (piÃ¹ permissivo)
        for key, voce_originale in self.voce_originale.items():
            voce_lower = voce_originale.lower()
            ...
            if parole_nome and parole_voce:
                ...
                if percentuale >= 0.7:  # 70% di sovrapposizione
                    print(f"   âœ… MATCH PARZIALE ({percentuale:.1%}): {key} â†’ '{voce_originale}'")
                    return key
        ...
        for key, voce_originale in self.voce_originale.items():
            ...
            if nome_lower in voce_lower or voce_lower in nome_lower:
                if len(nome_lower) >= 4:  # Evita match troppo corti
                    print(f"   âœ… MATCH SOTTOSTRINGA: {key} â†’ '{voce_originale}'")
                    return key
        
        print(f"   âŒ Nessuna corrispondenza trovata per: '{nome_prodotto}'")
        
        # DEBUG: Stampa tutte le voci disponibili
        print(f"   ðŸ“‹ Voci disponibili nel menu:")
    ...
    def salva_ordine(self, ordine):
        """Salva ordine nel file JSON - VERSIONE CORRETTA PER MODALITÃ€ MODIFICA"""
        import json
        
        ORDINI_FILE = gestore_percorsi.get_percorso("ordini_json")
        
        print(f"\nðŸ” === DEBUG SALVA_ORDINE MODALITÃ€ MODIFICA ===")
        print(f"ðŸ”§ modalita_modifica: {getattr(self, 'modalita_modifica', 'NON DEFINITA')}")
        print(f"ðŸ“‹ ordine_da_modificare presente: {hasattr(self, 'ordine_da_modificare') and self.ordine_da_modificare is not None}")
        print(f"ðŸ”¥ FIREBASE DEBUG - Ordine da salvare:")
        print(f"   - Intolleranze: {ordine.get('intolleranze', 'MANCANTI')}")
        print(f"   - Keys ordine: {list(ordine.keys())}")
        if not os.path.exists(ORDINI_FILE):
            ordini = []
            print("ðŸ“ File ordini.json non esiste, creo lista vuota")
        else:
            with open(ORDINI_FILE, "r", encoding="utf-8") as f:
                ordini = json.load(f)
            print(f"ðŸ“ Caricati {len(ordini)} ordini dal file")

        # ========== MODALITÃ€ MODIFICA: ALGORITMO MIGLIORATO ==========
        if hasattr(self, 'modalita_modifica') and self.modalita_modifica and hasattr(self, 'ordine_da_modificare') and self.ordine_da_modificare:
            print("ðŸ”„ MODALITÃ€ MODIFICA CONFERMATA: Cercando ordine da sostituire...")
            
            ordine_originale = self.ordine_da_modificare
            
            # Debug ordine originale
            print(f"ðŸŽ¯ ORDINE ORIGINALE:")
            ...
            if hasattr(self, 'indice_ordine_originale'):
                try:
                    if 0 <= self.indice_ordine_originale < len(ordini):
                        ...
                        print(f"âœ… TROVATO PER INDICE: posizione {self.indice_ordine_originale}")
                except:
                    pass
            
            # STRATEGIA 2: Match per combinazione Nome + Data + Cellulare (piÃ¹ affidabile)
            if not ordine_trovato:
                ...
                print(f"ðŸ” STRATEGIA 2: Cercando Nome='{nome_orig}' + Data='{data_orig}' + Cell='{cell_orig}'")
                ...
                for i, ordine_esistente in enumerate(ordini):
                    ...
                    if (nome_esist == nome_orig and 
                        ...
                        print(f"âœ… TROVATO PER DATI CLIENTE! Sostituendo ordine in posizione {i}")
                        ordini[i] = ordine
                        ordine_trovato = True
                        indice_trovato = i
                        break
            ...
            if not ordine_trovato:
                ...
                print(f"ðŸ” STRATEGIA 3: Cercando path: '{path_originale}'")
                ...
                for i, ordine_esistente in enumerate(ordini):
                    ...
                    if path_esistente == path_originale and path_originale != "":
                        print(f"âœ… TROVATO PER PATH! Sostituendo ordine in posizione {i}")
                        ordini[i] = ordine
                        ordine_trovato = True
                        indice_trovato = i
                        break
            ...
            if not ordine_trovato:
                ...
                print(f"ðŸ” STRATEGIA 4: Cercando solo Nome='{nome_orig}' + Data='{data_orig}'")
                ...
                for i, ordine_esistente in enumerate(ordini):
                    ...
                    if nome_esist == nome_orig and data_esist == data_orig:
                        print(f"âœ… TROVATO PER NOME+DATA! Sostituendo ordine in posizione {i}")
                        ordini[i] = ordine
                        ordine_trovato = True
                        indice_trovato = i
                        break
            ...
            if not ordine_trovato:
                print("âŒ ORDINE ORIGINALE NON TROVATO CON NESSUNA STRATEGIA!")
                print("   Aggiungendo come nuovo ordine...")
                ordini.append(ordine)
                indice_trovato = len(ordini) - 1
            else:
                print(f"ðŸŽ‰ MODIFICA COMPLETATA! Ordine aggiornato in posizione {indice_trovato}")
        
        else:
            # MODALITÃ€ NORMALE: Aggiungi nuovo ordine
            print("âž• MODALITÃ€ NORMALE: Aggiungendo nuovo ordine")
            ordini.append(ordine)
        ...
        try:
            ...
            print(f"ðŸ’¾ Archivio aggiornato - Totale ordini: {len(ordini)}")
            
        except Exception as e:
            print(f"âŒ ERRORE SALVATAGGIO: {e}")
        ...
        global archivio_dialogs_aperti
        print(f"ðŸ“Š Archivi aperti da aggiornare: {len(archivio_dialogs_aperti)}")
        
        for dialog in archivio_dialogs_aperti:
            try:
                dialog.ricarica_dati()
                print(f"ðŸ”„ Archivio dialog aggiornato")
            except Exception as e:
                print(f"âš ï¸ Errore aggiornamento archivio dialog: {e}")
        
        # MEMORIZZA che c'Ã¨ stata una modifica per archivi futuri
        if hasattr(self, 'modalita_modifica') and self.modalita_modifica:
            self._ordine_appena_modificato = True
            print("ðŸ’¡ Flag modifica impostato per prossimi archivi")
    ...
    def ricarica_dati_debug(self):
        ...
        print("ðŸ”„ === INIZIO RICARICAMENTO ARCHIVIO DEBUG ===")
        ...
        if current_item and current_item.parent() is not None:
            selected_cliente = current_item.text(1).replace("ðŸ‘¤ ", "")
            selected_data = current_item.text(0)
            selected_index = current_item.data(0, Qt.ItemDataRole.UserRole)
            print(f"ðŸ’¾ Elemento selezionato: {selected_cliente} - {selected_data} (idx: {selected_index})")
        ...
        try:
            self.load_orders()
            print(f"âœ… Ricaricati {len(getattr(self, 'ordini', []))} ordini")
        except Exception as e:
            print(f"âŒ Errore ricaricamento: {e}")
            return
        ...
        if selected_cliente and selected_data:
            ...
            for i in range(root.childCount()):
                ...
                for j in range(date_group.childCount()):
                    order_item = date_group.child(j)
                    item_cliente = order_item.text(1).replace("ðŸ‘¤ ", "")
                    item_data = order_item.text(0)
                    
                    if item_cliente == selected_cliente and item_data == selected_data:
                        self.tree.setCurrentItem(order_item)
                        self.tree.scrollToItem(order_item)
                        print(f"âœ… Selezione ristabilita per: {selected_cliente}")
                        break
        
        print("âœ… === FINE RICARICAMENTO ARCHIVIO DEBUG ===")
    ...
    def load_order_data(self, ordine):
        ...
        print(f"ðŸ“¥ Caricando ordine: {ordine['campi'].get('Nome', 'Cliente')}")
        ...
        print(f"ðŸŒ± Ripristinando intolleranze: {list(intolleranze_salvate.keys())}")
        
        # Prima azzera tutti i widget menu extra
        if hasattr(self, 'menu_extra_widgets'):
            for menu_type, widget in self.menu_extra_widgets.items():
                widget.setValue(0)  # Azzera tutti
                print(f"   ðŸ”„ Azzerato {menu_type}")
        ...
        for menu_type, dati_intolleranza in intolleranze_salvate.items():
            ...
            if hasattr(self, 'menu_extra_widgets') and menu_type in self.menu_extra_widgets:
                ...
                print(f"   âœ… Ripristinato {menu_type}: {persone_salvate} persone")
            else:
                print(f"   âš ï¸ Widget non trovato per {menu_type}")
        ...
        # SE SIAMO IN MODALITÃ€ MODIFICA, NON MOSTRARE IL MESSAGGIO
        if not (hasattr(self, 'modalita_modifica') and self.modalita_modifica):
            ...
            print(f"âœ… Ordine caricato per modifica: {ordine['campi'].get('Nome', 'Cliente')}")
    # --- Helper: riconoscimento torta (unica veritÃ ) ---
    ...
    def calcola_quantita(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap):
        voce = self.voce_originale.get(key, "").strip().lower()
        if "kg torta" in voce:
            print(f"ðŸŽ‚ TORTA: {num_pers} persone â†’ {num_pers/10:.1f} kg")
        ...
        def trova_dati_referenza(voce, dizionario, default):
            ...
            if any(x in voce for x in ["mozzarelline", "ricotta", "millefoglie"]):
                print(f"ðŸ” DEBUG MATCHING per '{voce[:30]}...':")
            ...
            for chiave in dizionario:
                ...
                if chiave_lower == voce_originale_lower:
                    if any(x in voce for x in ["mozzarelline", "ricotta", "millefoglie"]):
                        print(f"   âœ… MATCH ESATTO CON PARENTESI: trovato!")
            ...
            for chiave in dizionario:
                ...
                if chiave_norm == voce_norm:
                    if any(x in voce for x in ["mozzarelline", "ricotta", "millefoglie"]):
                        print(f"   âœ… MATCH SENZA PARENTESI: '{chiave_norm}' == '{voce_norm}'")
                    return dizionario[chiave]
            ...
            if "spicchietti" in voce_norm and "crostat" in voce_norm:
                print("   ðŸ”§ USANDO FALLBACK per spicchietti crostate")
                return {"difficolta": 1.0, "peso_unitario": 0.025}
            
            if any(x in voce for x in ["mozzarelline", "ricotta", "millefoglie"]):
                print(f"   âŒ NESSUN MATCH TROVATO - uso default")
            
            return default
        is_servito_service = False
        for k, checkbox in self.selected.items():
            if hasattr(checkbox, 'isChecked') and checkbox.isChecked():
                # RILEVA SERVITO SOLO SE LA CHIAVE INIZIA CON "servito|"
                if k.startswith("servito|"):
                    is_servito_service = True
                    print(f"   ðŸ†• RILEVATO SERVIZIO 'SERVITO' dalla sezione: {k}")
                    break
        
        # ========== GESTIONE SERVIZIO "SERVITO" - STRATEGIA SEMPLIFICATA ==========
        # ========== GESTIONE SERVIZIO "SERVITO" CON DIFFICOLTÃ€ ==========
        # ========== GESTIONE SERVIZIO "SERVITO" CON REGOLE INTEGRATE ==========
        if is_servito_service:
            print(f"\nðŸ½ï¸ APPLICANDO LOGICA 'SERVITO' CON DIFFICOLTÃ€ per: {voce}")
            print(f"   ðŸ‘¥ Persone: {num_pers}")
            
            # Ottieni dati difficoltÃ  per questa voce
            dati = trova_dati_referenza(voce, self.dati_referenze, {"difficolta": 1.0, "peso_unitario": 1.0})
            coeff = dati["difficolta"]
            peso_unitario = dati["peso_unitario"]
            
            print(f"   âš–ï¸ Coefficiente difficoltÃ : {coeff}")
            print(f"   ðŸ“ Peso unitario: {peso_unitario}")
            
            # ========== PRIMI ELABORATI - USA LOGICHE ESISTENTI ==========
            if key in self.referenze_primi:
                import math
                
                # Identifica tipologia primo
                primi_elaborati_keywords = ["lasagne", "cannelloni", "rosette", "nidi", "crespelle"]
                
                if any(pe in voce.lower() for pe in primi_elaborati_keywords):
                    print(f"   ðŸ PRIMO ELABORATO nel servito: {voce}")
                    
                    if "lasagne" in voce:
                        # Servito: quantitÃ  maggiorate (+30% rispetto al lunch)
                        teglie = max(1, math.ceil(num_pers / 20.0))  # Era 20 nel lunch, ora 15
                        print(f"   ðŸ Lasagne servito: {teglie} teglie")
                        return f"{int(teglie)} lasagne"
                    
                    elif "crespelle" in voce:
                        crespelle = max(12, math.ceil((num_pers * 0.9) / 8.0) * 8)  # Era 0.64, ora 0.8
                        print(f"   ðŸ¥Ÿ Crespelle servito: {crespelle}")
                        return f"{int(crespelle)} crespelle"
                    
                    elif "cannelloni" in voce:
                        cannelloni = max(12, math.ceil((num_pers * 0.9) / 12.0) * 12)
                        print(f"   ðŸ¥˜ Cannelloni servito: {cannelloni}")
                        return f"{int(cannelloni)} cannelloni"
                    
                    elif "nidi" in voce or "rosette" in voce:
                        tipo = 'nidi' if 'nidi' in voce else 'rosette'
                        pezzi = max(18, math.ceil((num_pers * 0.9) / 18.0) * 18)
                        print(f"   ðŸœ {tipo.title()} servito: {pezzi}")
                        return f"{int(pezzi)} {tipo}"
                
                else:
                    # PRIMI NORMALI nel servito - quantitÃ  maggiorate
                    print(f"   ðŸœ PRIMO NORMALE nel servito: {voce}")
                    peso_primi_totale = 0.220 * num_pers  # +25% rispetto ai 200g del lunch
                    
                    pesi_difficalta = [
                        self.dati_referenze.get(
                            self.voce_originale.get(k, "").strip().lower(), {"difficolta": 1.0}
                        )["difficolta"] for k in self.referenze_primi
                    ]
                    somma_coeff = sum(pesi_difficalta)
                    quota = coeff / somma_coeff if somma_coeff > 0 else 1 / len(self.referenze_primi)
                    peso_per_primo = peso_primi_totale * quota
                    ...
                    # APPLICA DIMEZZAMENTO SE NECESSARIO
                    primi_crudi = [
                        "maccheroncini pasticciati al ragÃ¹ bolognese(inox)*",
                    ...
                    if is_primo_crudo:
                        ...
                        print(f"   ðŸ Primo crudo (servito) DIMEZZATO: {peso_finale:.1f} kg")
                    
                    return f"{peso_finale:.1f} kg"
            
            # ========== DOLCI nel servito ==========
            elif key in self.referenze_dolci:
                peso_dolci_per_persona = 0.100  # 100g per persona nel servito (+25% rispetto ai 80g del lunch)
                ...
                def arrotonda_blocchi_servito(x, base):
                    x = int(round(x))
                    resto = x % base
                    if resto >= base * 0.6:
                        return x + (base - resto)
                ...
                return arrotonda_blocchi_servito(pezzi, 5)
            
            elif key in self.referenze_contorni:
                print(f"ðŸ¥— DEBUG CONTORNI per {voce}:")
                print(f"   - Persone: {num_pers}")
                print(f"   - Peso unitario: {peso_unitario}")
                
                # CONTORNI: 90g per persona
                peso_contorni_per_persona = 0.080
                ...
                print(f"   - Peso contorni totale: {peso_contorni_totale:.3f} kg")
                print(f"   - Numero contorni: {len(self.referenze_contorni)}")
                print(f"   - Peso per questo contorno: {peso_per_contorno:.3f} kg")
                
                # Converti in pezzi usando peso_unitario (0.5 per insalate)
                pezzi = peso_per_contorno / peso_unitario
                pezzi_finali = max(1, int(round(pezzi)))
                print(f"   - Pezzi calcolati: {pezzi:.1f} â†’ {pezzi_finali}")
                return pezzi_finali
            elif key in self.referenze_accompagnamenti:
                print(f"ðŸž DEBUG ACCOMPAGNAMENTI per {voce}:")
                print(f"   - Persone: {num_pers}")
                
                # Calcolo specifico per ogni accompagnamento
                if "grissini" in voce.lower():
                    # Ogni 10 persone = 5 grissini
                    # Formula: num_pers * 0.5
                    pezzi = int(num_pers * 0.5)
                    print(f"   ðŸ¥– Grissini: {num_pers} persone Ã— 0.5 = {pezzi} pezzi")
                    return pezzi
                
                elif "bocconcini alle olive" in voce.lower():
                    # Ogni 10 persone = 5 bocconcini  
                    # Formula: num_pers * 0.5
                    pezzi = int(num_pers * 0.5)
                    print(f"   ðŸ«’ Bocconcini alle olive: {num_pers} persone Ã— 0.5 = {pezzi} pezzi")
                    return pezzi
                
                elif "pane a fette" in voce.lower():
                    # Ogni 10 persone = 0.5 kg
                    # Formula: num_pers * 0.05
                    kg = num_pers * 0.05
                    ...
                    if kg <= int(kg) + 0.25:
                        ...
                        kg_finali = int(kg) + 0.5
                    else:
                        kg_finali = int(kg) + 1
                    
                    kg_finali = max(0.5, kg_finali)  # Minimo 0.5 kg
                    
                    print(f"   ðŸž Pane a fette: {num_pers} persone Ã— 0.05 = {kg:.2f} kg â†’ {kg_finali} kg")
                ...
                # Fallback per altri accompagnamenti
                    ...
                    print(f"   âš ï¸ Accompagnamento non riconosciuto: {voce}")
                    return 1
            elif key in self.referenze_secondi:
                print(f"ðŸ– DEBUG SECONDI per {voce}:")
                print(f"   - Persone: {num_pers}")
                print(f"   - Peso unitario: {peso_unitario}")
                
                # SECONDI: 150g per persona 
                peso_secondi_per_persona = 0.150
                ...
                print(f"   - Peso secondi totale: {peso_secondi_totale:.3f} kg")
                print(f"   - Numero secondi: {len(self.referenze_secondi)}")
                print(f"   - Peso per questo secondo: {peso_per_secondo:.3f} kg")
                
                # Converti in pezzi usando peso_unitario (1.5 kg per millefoglie)
                pezzi = peso_per_secondo / peso_unitario
                pezzi_finali = max(1, int(round(pezzi)))
                print(f"   - Pezzi calcolati: {pezzi:.1f} â†’ {pezzi_finali}")
                return pezzi_finali
            # ========== SALATO nel servito - USA LOGICHE REFERENZE IN KG ==========
            elif key in self.referenze_salato:
                peso_salato_per_persona = 0.200  # 200g salato per persona nel servito
                
                pesi_difficalta = [
                    self.dati_referenze.get(
                        self.voce_originale.get(k, "").strip().lower(), {"difficolta": 1.0}
                    )["difficolta"] for k in self.referenze_salato
                ]
                somma_coeff = sum(pesi_difficalta)
                quota = coeff / somma_coeff if somma_coeff > 0 else 1 / len(self.referenze_salato)
                peso_salato = peso_salato_per_persona * num_pers
                peso_target = peso_salato * quota
                
                # USA LE STESSE REGOLE DEL LUNCH: referenze in kg vs pezzi
                referenze_in_kg = [
                # FINGER FOOD E DOLCI (mantieni quelli che erano giÃ  in kg)
                ...
                va_in_kg = any(ref in voce for ref in referenze_in_kg)
                print(f"   ðŸ½ï¸ Salato servito - Va in kg: {va_in_kg}")
                ...
                if va_in_kg:
                    ...
                    if peso_unitario >= 0.5:
                        ...
                        print(f"   ðŸ¥¬ Prodotto pesante: {risultato_pezzi} pezzi (no blocchi)")
                    else:
                        def arrotonda_blocchi(x, base):
                            x = int(round(x))
                            resto = x % base
                            return x + (base - resto) if resto >= base * 0.6 else x - resto
                        risultato_pezzi = arrotonda_blocchi(pezzi, 5)

                    return risultato_pezzi


        # DEBUG
        print(f"ðŸ” DEBUG JSON per {voce}:")
        print(f"   - self.dati_referenze Ã¨ None? {self.dati_referenze is None}")
        print(f"   - Dimensione dati_referenze: {len(self.dati_referenze) if self.dati_referenze else 0}")
        if self.dati_referenze:
            chiavi_esempio = list(self.dati_referenze.keys())[:5]
            print(f"   - Prime 5 chiavi: {chiavi_esempio}")
            pizzette_keys = [k for k in self.dati_referenze.keys() if "pizzette" in k]
            print(f"   - Chiavi con 'pizzette': {pizzette_keys}")

        # FUNZIONE DI MATCH ROBUSTO
        

        dati = trova_dati_referenza(voce, self.dati_referenze, {"difficolta": 1.0, "peso_unitario": 1.0})

        # ========== LOGICA MINI CALZONCINI 2:1 (POSIZIONATA CORRETTAMENTE) ==========
        print(f"   - MATCH TROVATO? {[k.strip().lower() for k in self.dati_referenze].count(voce)} volte")
        print(f"âž¡ï¸ PESO UNITARIO USATO: {dati['peso_unitario']}")

        coeff = dati["difficolta"]
        peso_unitario = dati["peso_unitario"]
        
        
        # ========== FINE LOGICA MINI CALZONCINI ==========
        # DEBUG
        print(f"ðŸ” DEBUG JSON per {voce}:")
        print(f"   - self.dati_referenze Ã¨ None? {self.dati_referenze is None}")
        print(f"   - Dimensione dati_referenze: {len(self.dati_referenze) if self.dati_referenze else 0}")
        ...
        # FUNZIONE DI MATCH ROBUSTO
        # FUNZIONE DI MATCH ROBUSTO MIGLIORATA
        import re
        ...
        # ========== LOGICA FIERA CORRETTA ==========
        # ========== LOGICA FIERA UNIFICATA - 200G PER PERSONA PER TUTTO ==========
        if getattr(self, 'evento_in_fiera', False):
            print(f"ðŸ¢ CALCOLO FIERA per {voce} - {num_pers} persone")
            
            # FIERA: SEMPRE 200g per persona, indipendentemente dal servizio
            peso_totale_fiera = num_pers * 0.2  # 200g per persona
            
            # Gestione speciale per primi elaborati
            if key in self.referenze_primi:
                import math
                if any(x in voce for x in ["lasagne", "cannelloni", "rosette", "nidi", "crespelle"]):
                    # Primi elaborati: quantitÃ  fisse per fiera (piÃ¹ conservative)
                    if "lasagne" in voce:
                        teglie = max(1, math.ceil(num_pers / 25.0))  # Meno teglie per fiera
                        print(f"   ðŸ Lasagne fiera: {teglie} teglie")
                        return f"{int(teglie)} lasagne"
                    elif "crespelle" in voce:
                        crespelle = max(8, math.ceil(num_pers / 12.0) * 8)  # Meno crespelle
                        print(f"   ðŸ Crespelle fiera: {crespelle}")
                        return f"{int(crespelle)} crespelle"
                    elif "cannelloni" in voce:
                        cannelloni = max(12, math.ceil(num_pers / 15.0) * 12)
                        print(f"   ðŸ Cannelloni fiera: {cannelloni}")
                        return f"{int(cannelloni)} cannelloni"
                    elif "nidi" in voce:
                        nidi = max(18, math.ceil(num_pers / 25.0) * 18)
                        print(f"   ðŸ Nidi fiera: {nidi}")
                        return f"{int(nidi)} nidi"
                    elif "rosette" in voce:
                        rosette = max(18, math.ceil(num_pers / 25.0) * 18)
                        print(f"   ðŸ Rosette fiera: {rosette}")
                        return f"{int(rosette)} rosette"
            
            # Distribuzione proporzionale per tutti gli altri prodotti
            pesi_difficalta = [
                self.dati_referenze.get(
                    self.voce_originale.get(k, "").strip().lower(), {"difficolta": 1.0}
                )["difficolta"] for k in referenze_cibo
            ]
            somma_coeff = sum(pesi_difficalta)
            quota = coeff / somma_coeff if somma_coeff > 0 else 1 / len(referenze_cibo)
            peso_target = peso_totale_fiera * quota
            
            # Converti in pezzi o mantieni peso
            if peso_unitario >= 0.5:  # Prodotti pesanti (Verdi, Quiche)
                pezzi = max(1, int(round(peso_target / peso_unitario)))
                print(f"   ðŸ¢ Fiera prodotto pesante: {pezzi} pezzi")
                return pezzi
            else:
                pezzi = peso_target / peso_unitario
                risultato = max(1, int(round(pezzi)))  # Minimo 1 pezzo
                print(f"   ðŸ¢ Fiera: {risultato} pezzi (peso target: {peso_target:.3f}kg)")
                return risultato

        # ========== LOGICA STANDARD (INVARIATA) ==========
        is_ap_con_primi = "aperitivo" in servizio_lower and getattr(self, 'ap_buffet_tipo', 'Leggero').lower() == "rinforzato" and key in self.referenze_primi
        is_extra_primo = key in self.referenze_primi and "extra" in key.lower()
        if ("mezze maniche" in voce or "insalata di riso" in voce or "penne" in voce or "orecchiette" in voce):
            print(f"\nðŸš¨ DEBUG PRIMO APERITIVO RINFORZATO: {voce}")
            print(f"   - key: '{key}'")
            print(f"   - servizio_lower: '{servizio_lower}'")
            print(f"   - tipo_ap: '{tipo_ap}'")
            print(f"   - getattr(self, 'ap_buffet_tipo'): '{getattr(self, 'ap_buffet_tipo', 'NON_TROVATO')}'")
            print(f"   - key in self.referenze_primi: {key in self.referenze_primi}")
            print(f"   - is_ap_con_primi: {is_ap_con_primi}")
            print(f"   - is_extra_primo: {is_extra_primo}")
            print(f"   - self.referenze_primi: {self.referenze_primi}")
            print(f"   - len(self.referenze_primi): {len(self.referenze_primi) if self.referenze_primi else 0}")

        if "aperitivo" in servizio_lower and not is_ap_con_primi:
            # APERITIVO RINFORZATO: usa stessa logica del lunch per contorni e accompagnamenti
            if tipo_ap == "rinforzato" and (key in self.referenze_contorni or key in self.referenze_accompagnamenti):
                print(f"ðŸ· APERITIVO RINFORZATO - USANDO LOGICA LUNCH per {voce}:")
                
                # ACCOMPAGNAMENTI: stessa logica del lunch
                ...
                # CONTORNI: stessa logica del lunch  
                elif key in self.referenze_contorni:
                    peso_contorni_per_persona = 0.080
                    peso_contorni_totale = peso_contorni_per_persona * num_pers
                    ...
                    print(f"   ðŸ¥— Contorno aperitivo rinforzato: {peso_per_contorno:.3f} kg")
                    
                    # Converti in pezzi usando peso_unitario
                    pezzi = peso_per_contorno / peso_unitario
                    pezzi_finali = max(1, int(round(pezzi)))
                    print(f"   - Pezzi calcolati: {pezzi:.1f} â†’ {pezzi_finali}")
                    return pezzi_finali
            
            # LOGICA ORIGINALE per tutto il resto
            # QuantitÃ  corrette da quantitÃ .txt: Leggero 230g, Rinforzato 350g
            peso_totale_persona = 0.350 if tipo_ap == "rinforzato" else 0.230
            peso_totale = num_pers * peso_totale_persona
            
            print(f"ðŸ· DEBUG APERITIVO per {voce}:")
            print(f"   - Tipo: {tipo_ap}")
            print(f"   - Peso per persona: {peso_totale_persona*1000}g")
            print(f"   - Peso totale: {peso_totale:.3f} kg")
            ...
            somma_coeff = sum(pesi_difficalta)
            quota = coeff / somma_coeff if somma_coeff > 0 else 1 / len(referenze_cibo)
            peso_target = peso_totale * quota
            
            # Converti in pezzi usando peso unitario
            pezzi = peso_target / peso_unitario
            risultato = int(round(pezzi))
            
            print(f"   - Quota difficoltÃ : {quota:.3f}")
            print(f"   - Peso target: {peso_target:.3f} kg")
            print(f"   - Pezzi calcolati: {risultato}")
            
            return risultato
        
        if "lunch box" in servizio_lower:
            print(f"ðŸ“¦ DEBUG LUNCH BOX per {voce}:")
            voce_lunch_lower = voce.lower()
            # Per primi piatti con (vasch) o (stagn.mono)  
            if ("(vasch)" in voce_lunch_lower or "(stagn.mono)" in voce_lunch_lower):
                # Rimuovi (vasch) e (stagn.mono) dal nome ma mantieni resto del nome
                nome_pulito = voce.replace("(vasch)", "").replace("(stagn.mono)", "").strip()
                return f"NR {num_pers} porz {nome_pulito}"  # â† CORRETTO: "porz" invece di "mono"
            else:
                return f"({num_pers})1"

        elif "lunch" in servizio_lower or is_ap_con_primi or is_extra_primo:
            # QuantitÃ  corrette da quantitÃ .txt:
            # - Lunch con primi: 420g (200g primi + 220g resto)  
            # - Lunch senza primi: 350g totali
            # - Aperitivo rinforzato con primi: usa stessa logica lunch con primi

            ha_primi = bool(self.referenze_primi) and len(self.referenze_primi) > 0
            peso_totale = 0.420 * num_pers if ha_primi else 0.400 * num_pers

            print(f"ðŸ½ï¸ DEBUG LUNCH per {voce}:")
            print(f"   - Ha primi: {ha_primi}")
            print(f"   - Peso totale: {peso_totale:.3f} kg")
            print(f"   - Referenze primi: {self.referenze_primi if hasattr(self, 'referenze_primi') else 'N/A'}")

            if key in self.referenze_primi:
                import math
                
                # 1. IDENTIFICA TUTTI I PRIMI E LA LORO TIPOLOGIA
                ...
                print(f"ðŸ CLASSIFICAZIONE PRIMI:")
                print(f"   - Freddi: {len(primi_freddi)} ({primi_freddi})")
                print(f"   - Elaborati: {len(primi_elaborati)} ({primi_elaborati})")
                print(f"   - Normali: {len(primi_normali)} ({primi_normali})")
                
                # 2. CALCOLA PESO TOTALE PRIMI (200g per persona)
                # 2. CALCOLA PESO TOTALE PRIMI (160g per persona)
                peso_primi_totale = 0.160 * num_pers

                # 3. CALCOLA PRIMA IL PESO DEI PRIMI ELABORATI
                ...
                # 4. CALCOLA PESO RIMANENTE PER FREDDI E NORMALI
                peso_rimanente = max(0.5, peso_primi_totale - peso_consumato_elaborati)
                ...
                print(f"   ðŸ“Š CALCOLO BILANCIATO:")
                print(f"   - Peso totale primi: {peso_primi_totale:.1f} kg")
                ...
                # 5. GESTISCI OGNI TIPOLOGIA
                if key in primi_freddi:
                    # PRIMI FREDDI: usano il peso rimanente
                    ...
                    print(f"   ðŸ¥— Primo freddo BILANCIATO: {peso_finale:.1f} kg")
                    return f"{peso_finale:.1f} kg"

                elif key in primi_elaborati:
                    # PRIMI ELABORATI: usa quantitÃ  giÃ  calcolate
                    import math
                    
                    if "lasagne" in voce:
                        teglie = max(1, math.ceil(num_pers / 20.0)) if len(self.referenze_primi) > 1 else math.ceil(num_pers / 10.0)
                        print(f"   ðŸ Lasagne BILANCIATO: {teglie} teglie")
                        return f"{int(teglie)} lasagne"
                    
                    elif "crespelle" in voce:
                        crespelle = max(8, math.ceil((num_pers * 0.64) / 8.0) * 8) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 2) / 8.0) * 8
                        print(f"   ðŸ¥Ÿ Crespelle BILANCIATO: {crespelle}")
                        return f"{int(crespelle)} crespelle"
                    
                    elif "cannelloni" in voce:
                        cannelloni = max(12, math.ceil((num_pers * 0.64) / 12.0) * 12) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 6.0) * 12
                        print(f"   ðŸ¥˜ Cannelloni BILANCIATO: {cannelloni}")
                        return f"{int(cannelloni)} cannelloni"
                    
                    elif "nidi" in voce or "rosette" in voce:
                        tipo = 'nidi' if 'nidi' in voce else 'rosette'
                        pezzi = max(18, math.ceil((num_pers * 0.64) / 18.0) * 18) if len(self.referenze_primi) > 1 else math.ceil((num_pers * 0.64) / 9.0) * 18
                        print(f"   ðŸœ {tipo.title()} BILANCIATO: {pezzi}")
                        return f"{int(pezzi)} {tipo}"

                else:
                # PRIMI NORMALI: dividono il peso rimasto
                    peso_per_primo_normale = peso_per_primo_non_elaborato
                    
                    intero = int(peso_per_primo_normale)
                    decimale = peso_per_primo_normale - intero
                    if decimale < 0.2:
                        peso_finale = float(intero)
                    elif 0.2 <= decimale < 0.7:
                        peso_finale = intero + 0.5
                    ...
                    # ========== NUOVA LOGICA PER PRIMI CRUDI (inox) ==========
                    # Lista dei primi che devono essere dimezzati (peso crudo)
                    primi_crudi = [
                        "maccheroncini pasticciati al ragÃ¹ bolognese(inox)*",
                        "paccheri con fiammiferi di speck croccante e zucchine marinate(inox)*", 
                        "gnocchetti sardi con pesto di noci e radicchio di treviso(inox)*",
                    ...
                    if is_primo_crudo:
                        ...
                        peso_finale = peso_finale / 2
                        print(f"   ðŸ Primo crudo (inox) DIMEZZATO: {peso_finale:.1f} kg (da cuocere)")
                        ...
                        print(f"   ðŸœ Primo normale: {peso_finale:.1f} kg (su {len(self.referenze_primi)} primi totali)")
                        
                    return f"{peso_finale:.1f} kg"
            if "taglieri" in voce.lower():
                print(f"ðŸ§€ DEBUG TAGLIERI per {voce}:")
                print(f"   - Persone: {num_pers}")
                
                if "taglieri di salumi medi" in voce.lower():
                    # 1 tagliere ogni 20 persone
                    num_taglieri = max(1, int(num_pers / 20))
                    print(f"   ðŸ¥“ Taglieri salumi: {num_pers} persone Ã· 20 = {num_taglieri} taglieri")
                    
                    # Auto-selezione accompagnamenti (da gestire nell'interfaccia)
                    # Qui calcoliamo solo i taglieri
                    return num_taglieri
                
                elif "taglieri formaggi piccoli" in voce.lower():
                    # 1 tagliere ogni 20 persone  
                    num_taglieri = max(1, int(num_pers / 20))
                    print(f"   ðŸ§€ Taglieri formaggi: {num_pers} persone Ã· 20 = {num_taglieri} taglieri")
                    
                    # Auto-selezione accompagnamenti (da gestire nell'interfaccia)
                    # Qui calcoliamo solo i taglieri
                    return num_taglieri

            # ========== LOGICA PER ACCOMPAGNAMENTI TAGLIERI ==========
            # Gestisce miele, composta di frutta quando sono selezionati insieme ai taglieri formaggi
            if any(voce_item in voce.lower() for voce_item in ["miele", "composta di frutta", "composte di frutta"]):
                # Controlla se Ã¨ selezionato "Taglieri formaggi piccoli"
                ...
                if taglieri_formaggi_selezionati:
                    ...
                    print(f"   ðŸ¯ {voce} per taglieri: {num_accompagnamenti} pz")
                    return num_accompagnamenti

            # Gestisce crescenta e crescentine per taglieri salumi
            if any(voce_item in voce.lower() for voce_item in ["mini crescentine fritte"]):
                # Controlla se Ã¨ selezionato "Taglieri di salumi medi"
                ...
                if taglieri_salumi_selezionati:
                    if "mini crescentine fritte" in voce.lower():
                        # Crescentine: 1 per persona
                        print(f"   ðŸ¥® Crescentine: {num_pers} pz (1 per persona)")
                        return num_pers
            elif key in self.referenze_dolci:
                ...
                if len(bicchierini_selezionati) >= 2 and len(altri_dolci_selezionati) == 0:
                    if key in bicchierini_selezionati:
                        ...
                        print(f"   ðŸ® Solo bicchierini: {risultato} porzioni")
                        return risultato
                
                # Caso 2: Un bicchierino + altri dolci (rapporto 40%-60%)
                    ...
                    if "aperitivo" in servizio_lower:
                        peso_dolci_per_persona = 0.070 if tipo_ap == "rinforzato" else 0.040
                    else:
                    ...
                    if key in bicchierini_selezionati:
                        # Bicchierino: 40% del peso totale dolci
                        peso_target = peso_totale_dolci * 0.35
                        print(f"   ðŸ® Bicchierino (40%): peso_target = {peso_target:.3f} kg")
                    else:
                        # Altri dolci: 60% diviso tra loro
                        peso_target = (peso_totale_dolci * 0.60) / len(altri_dolci_selezionati)
                        print(f"   ðŸ® Altro dolce (60%): peso_target = {peso_target:.3f} kg")
                ...
                # Caso 3: Logica standard (nessun bicchierino o solo altri dolci)
                    ...
                    pezzi = peso_target / peso_unitario
                    print(f"   ðŸ® Logica standard: {int(round(pezzi))} pezzi")
                ...
                if "grissini" in voce.lower():
                    ...
                    print(f"   ðŸž Pane a fette: {num_pers} persone Ã— 0.05 = {pz:.2f} pz â†’ {pz_finali} pz")
                ...
                print(f"ðŸ” DEBUG SALATO per {voce}:")
                pesi_difficalta = [
                    self.dati_referenze.get(
                        self.voce_originale.get(k, "").strip().lower(), {"difficolta": 1.0}
                    )["difficolta"] for k in self.referenze_salato
                ]
                print(f"   - Pesi difficoltÃ : {pesi_difficalta}")
                print(f"   - Coefficiente questa voce: {coeff}")
                
                somma_coeff = sum(pesi_difficalta)
                quota = coeff / somma_coeff if somma_coeff > 0 else 1 / len(self.referenze_salato)
                # Calcolo peso salato corretto:
                # - Se ha primi: 420g totali - 200g primi - 80g dolci = 140g salato
                # - Se non ha primi: 350g totali - 70g dolci = 280g salato
                # Calcolo peso salato corretto per aperitivo e lunch:
                # Scelta dei pesi per APERITIVO vs LUNCH
                if "aperitivo" in servizio_lower:
                    # Aperitivo: Leggero 40g dolci + 190g salato, Rinforzato 70g dolci + 280g salato
                    if tipo_ap == "rinforzato":
                        peso_dolci_per_persona = 0.070  # 70g
                        peso_salato_per_persona = 0.280 # 280g
                    else:
                        peso_dolci_per_persona = 0.040  # 40g
                        peso_salato_per_persona = 0.190 # 190g
                else:
                    # Lunch
                    if ha_primi:
                        peso_dolci_per_persona = 0.080  # 80g dolci con primi
                        peso_salato_per_persona = 0.180 # 180g salato con primi
                    else:
                        peso_dolci_per_persona = 0.070  # 70g dolci senza primi
                        peso_salato_per_persona = 0.350 # 350g salato senza primi

                peso_salato = peso_salato_per_persona * num_pers
                peso_target = peso_salato * quota

                print(f"   - Peso dolci per persona: {peso_dolci_per_persona*1000:.0f}g")
                print(f"   - Peso salato per persona: {peso_salato_per_persona*1000:.0f}g")
                print(f"   - Somma coefficienti: {somma_coeff}")
                print(f"   - Quota: {quota:.3f}")
                print(f"   - Peso salato totale: {peso_salato:.3f} kg")
                print(f"   - Peso target: {peso_target:.3f} kg")

                # DECISIONE: KG O PEZZI?
                referenze_in_kg = [
                    # FINGER FOOD E DOLCI (mantieni quelli giÃ  in kg)
                    "biscotteria","spicchietti crostate","trancetti tenerina crema e amerene","mini tramezzini vegetariani","rustici vegetariani",
                    "rustici mix","parmigianini","pasticceria","cubetti mortadella","cubetti mortadella scottati all'aceto",
                    "dadini di mortadella","rombetti di frittata","mini tramezzini mix","mozzarelline in carrozza","mini cotolettine",
                    "olive all'ascolana","olive ascolane","crocchette","tagliata di frutta","mozzarelle","polpettine prosciutto e ricotta",
                    "minisw farciti mix","trancetti tenerina cioccolato","crescenta a dadini",
                    # INSALATE / SECONDI FREDDI
                    "insalata di pollo","insalatona mista con ananas","insalata di pollo alla nizzarda","caprese","insalata di pollo marinato",
                    "tacchino con asparagi","straccetti con scorza","spezzatino con topinambur","bocconcini di tacchino","straccetti di pollo",
                    "spezzatino di maiale","bocconcini di maiale","rombetti di tacchino","piccatine al balsamico",
                    "filettino di maiale","bocconcini di pollo","rombi di tacchino","straccetti di maiale","filetto di tacchino",
                    # CONTORNI
                ...
                voce_lower = voce.lower()
                referenze_in_kg_lower = [ref.lower() for ref in referenze_in_kg]
                va_in_kg = any(ref in voce_lower for ref in referenze_in_kg_lower)
                print(f"   - Va in kg: {va_in_kg}")
                ...
                if va_in_kg:
                    ...
                    if not peso_unitario or peso_unitario <= 0:
                        print("   âš ï¸ peso_unitario nullo/<=0: ritorno 1 pezzo di sicurezza")
                        return 1

                    pezzi = peso_target / peso_unitario

                    # PRODOTTI PESANTI vs ALTRI â†’ definisci SEMPRE risultato_pezzi
                    if peso_unitario >= 0.5:
                        risultato_pezzi = max(1, int(round(pezzi)))
                        print(f"   ðŸ¥¬ Prodotto pesante: {risultato_pezzi} pezzi (no blocchi)")
                    else:
        ...
        elif "lunch box" in servizio_lower:
            print(f"ðŸ“¦ DEBUG LUNCH BOX per {voce}:")
            voce_lunch_lower = voce.lower()
            # Per primi piatti con (vasch) o (stagn.mono)
            if ("(vasch)" in voce_lunch_lower or "(stagn.mono)" in voce_lunch_lower):
                nome_pulito = voce.replace("(vasch)", "").replace("(stagn.mono)", "").strip()
                return f"NR {num_pers} mono{nome_pulito}"
            else:
                return 1
        elif "coffee" in servizio_lower or "tea" in servizio_lower:
            # QUANTITÃ€ CORRETTE: 80-100g per persona (media 90g)
            if getattr(self, 'evento_in_fiera', False):
                peso_totale = num_pers * 0.29  # 90g base + 200g fiera = 290g
            else:
                peso_totale = num_pers * 0.085  # 90g per persona
            
            print(f"â˜• DEBUG COFFEE per {voce}:")
            print(f"   - Persone: {num_pers}")
            print(f"   - Peso totale coffee: {peso_totale:.3f} kg")
            print(f"   - Coefficiente difficoltÃ : {coeff}")
            print(f"   - Peso unitario: {peso_unitario}")
            ...
            print(f"   - Quota: {quota:.3f}")
            print(f"   - Peso target: {peso_target:.3f} kg")
            print(f"   - Pezzi calcolati: {pezzi:.1f}")
            

        # Se arriviamo qui, usa la logica di arrotondamento standard
        def arrotonda_blocchi(x, base):
            x = int(round(x))
            resto = x % base
            if resto >= base * 0.6:
...
# ==============================================================================
    ...
    def gestisci_selezione_insalata(self, checkbox, key_insalata):
        ...
        # Lista delle insalate che richiedono condimenti automatici
        insalate_con_condimenti = [
            "insalatona verde mista con tonno",
            "insalatona verde mista con mozzarella", 
            "insalatona verde mista con tonno e uovo",
            "insalata waldorf con sedano e mele saltate in padella",
        ...
        # Verifica se questa voce Ã¨ una delle insalate speciali
        is_insalata_speciale = any(insalata in voce_insalata for insalata in insalate_con_condimenti)
        
        if is_insalata_speciale:
            print(f"ðŸ¥— Gestione insalata: {voce_insalata[:50]}...")
            print(f"   ðŸ” checkbox.isChecked() = {checkbox.isChecked()}")
            
            if checkbox.isChecked():
                print(f"   âž¡ï¸ Entrando nel ramo SELEZIONE")
                ...
                for k, cb in self.selected.items():
                    if hasattr(cb, 'isChecked'):
                        ...
                        if "condimento" in voce_condimenti and "insalata" in voce_condimenti:
                            print(f"   ðŸŽ¯ Trovato condimento: chiave={k}")
                            if not cb.isChecked():
                                cb.setChecked(True)
                                print(f"âœ… Auto-selezionato: {self.voce_originale.get(k, k)} (per insalata)")
                            else:
                                print(f"â„¹ï¸ Condimento giÃ  selezionato")
                            break
            else:
                print(f"   âž¡ï¸ Entrando nel ramo DESELEZIONE")
                ...
                for k, cb in self.selected.items():
                    if k != key_insalata and hasattr(cb, 'isChecked') and cb.isChecked():
                        ...
                        if any(ins in voce_altra for ins in insalate_con_condimenti):
                            altre_insalate_attive = True
                            print(f"   ðŸ” Trovata altra insalata attiva: {voce_altra[:30]}...")
                            break
                
                print(f"   ðŸ“Š Altre insalate attive: {altre_insalate_attive}")
                ...
                if not altre_insalate_attive:
                    for k, cb in self.selected.items():
                        if hasattr(cb, 'isChecked'):
                            ...
                            if "condimento" in voce_condimenti and "insalata" in voce_condimenti:
                                cb.setChecked(False)
                                print(f"âŒ Auto-deselezionato: {self.voce_originale.get(k, k)} (nessuna insalata attiva)")
                                break
                else:
                    print(f"â„¹ï¸ Condimento mantenuto (altre insalate ancora attive)")
    def gestisci_selezione_secondi_contorni(self, checkbox, key_secondo_contorno):
        ...
        # Verifica se questa voce Ã¨ un secondo o contorno
        ...
        if is_secondo_o_contorno:
            print(f"ðŸ– Gestione secondo/contorno: {voce_corrente[:50]}...")
            print(f"   ðŸ” checkbox.isChecked() = {checkbox.isChecked()}")
                        ...
                        if "optionals" in k.lower():
                            ...
                            for extra in extras_da_selezionare:
                                if extra in voce_extra:
                                    print(f"   ðŸŽ¯ Trovato extra: {extra} (chiave={k})")
                                    if not cb.isChecked():
                                        cb.setChecked(True)
                                        print(f"âœ… Auto-selezionato: {self.voce_originale.get(k, k)} (per secondo/contorno)")
                                    else:
                                        print(f"â„¹ï¸ Extra giÃ  selezionato: {extra}")
                                    break
            else:
                print(f"   âž¡ï¸ Entrando nel ramo DESELEZIONE")
                ...
                for k, cb in self.selected.items():
                    if k != key_secondo_contorno and hasattr(cb, 'isChecked') and cb.isChecked():
                        if ("secondi piatti" in k.lower() or "contorni" in k.lower()):
                            ...
                            print(f"   ðŸ” Trovato altro secondo/contorno attivo: {k[:30]}...")
                            break
                
                print(f"   ðŸ“Š Altri secondi/contorni attivi: {altri_secondi_contorni_attivi}")
                        ...
                        if hasattr(cb, 'isChecked'):
                            ...
                            if "optionals" in k.lower():
                                for extra in extras_da_selezionare:
                                    if extra in voce_extra:
                                        cb.setChecked(False)
                                        print(f"âŒ Auto-deselezionato: {self.voce_originale.get(k, k)} (nessun secondo/contorno attivo)")
                                        break
                else:
                    print(f"â„¹ï¸ Extra mantenuti (altri secondi/contorni ancora attivi)")
    def gestisci_selezione_bevanda(self, checkbox, key_bevanda):
        ...
        print(f"ðŸ· Gestione bevanda: {voce_bevanda[:50]}...")
        ...
        if len(parti_key) >= 2:
            ...
            print(f"   ðŸ” Sezione bevanda: '{sezione_bevanda}'")
        else:
            print(f"   âš ï¸ Formato key non riconosciuto: {key_bevanda}")
            return
            ...
            for k, cb in self.selected.items():
                if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                    ...
                    if len(parti_k) >= 2:
                        ...
                        # CONTROLLA SE L'ACCESSORIO Ãˆ NELLA STESSA SEZIONE DELLA BEVANDA
                        if sezione_accessorio != sezione_bevanda:
                            continue  # Salta accessori di altre sezioni
                    else:
                        continue  # Salta se il formato non Ã¨ riconosciuto
                    ...
                    if ("bowl" in voce_accessorio and "ice" in voce_accessorio) or "bowl+ice" in voce_accessorio:
                        if "+cons" in voce_bevanda:
                            cb.setChecked(True)
                            print(f"âœ… Auto-selezionato: {self.voce_originale.get(k, k)} (bevanda CONS, sezione: {sezione_accessorio})")
                    
                    # Flutes per Prosecco
                    if "prosecco" in voce_bevanda and "flutes" in voce_accessorio and "mono" in voce_accessorio:
                        cb.setChecked(True)
                        print(f"âœ… Auto-selezionato: {self.voce_originale.get(k, k)} (per Prosecco, sezione: {sezione_accessorio})")
                    ...
                    if ("chardonnay" in voce_bevanda or "cabernet" in voce_bevanda) and "calici" in voce_accessorio and "mono" in voce_accessorio:
                        cb.setChecked(True)
                        print(f"âœ… Auto-selezionato: {self.voce_originale.get(k, k)} (per vino, sezione: {sezione_accessorio})")
        else:
            ...
            for k, cb in self.selected.items():
                if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                    ...
                    if not altre_bevande_cons and (("bowl" in voce_accessorio and "ice" in voce_accessorio) or "bowl+ice" in voce_accessorio):
                        cb.setChecked(False)
                        print(f"âŒ Auto-deselezionato: {self.voce_originale.get(k, k)} (sezione: {sezione_accessorio})")
                    
                    # Flutes solo se non c'Ã¨ prosecco NELLA STESSA SEZIONE
    ...
    def gestisci_selezione_primo(self, checkbox, key_primo):
        ...
        # Determina se Ã¨ un primo caldo o freddo
        ...
        print(f"ðŸ Gestione primo: {voce_primo[:50]}... | Caldo: {is_primo_caldo}")
        
        # ========== ESTRAI LA SEZIONE DEL PRIMO ==========
        parti_key = key_primo.split('|')
        if len(parti_key) >= 2:
            sezione_primo = parti_key[0].lower()
            print(f"   ðŸ” Sezione primo: '{sezione_primo}'")
        else:
            print(f"   âš ï¸ Formato key non riconosciuto: {key_primo}")
            return
            ...
            for k, cb in self.selected.items():
                if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                    ...
                    if "acc x porz" in voce_accessorio or "acc." in voce_accessorio:
                        cb.setChecked(True)
                        print(f"âœ… Auto-selezionato: {self.voce_originale.get(k, k)} (sezione: {sezione_accessorio})")
                    
                    # "Scaldavivande" solo per primi caldi
                    if is_primo_caldo and "scaldavivande" in voce_accessorio:
                        cb.setChecked(True)
                        print(f"âœ… Auto-selezionato: {self.voce_originale.get(k, k)} (primo caldo, sezione: {sezione_accessorio})")
        else:
            ...
            for k, cb in self.selected.items():
                if k != key_primo and hasattr(cb, 'isChecked') and cb.isChecked():
                    ...
                    if categoria == "cibo":
                        ...
                        # Controlla se Ã¨ un primo piatto
    ...
    def determina_unita_misura(self, voce_originale, quantita):
        ...
        if isinstance(quantita, str) and quantita.startswith("(") and ")" in quantita:
            # Formato (50)1 â†’ estrai i numeri
            import re
        ...
        # ========== GESTIONE PRIMI ELABORATI (PRIORITÃ€ ALTA) ==========
        ...
        # ========== GESTIONE STRINGHE CON UNITÃ€ GIÃ€ DEFINITE ==========
        if isinstance(quantita, str):
            if "kg" in quantita.lower():
                return quantita  # GiÃ  formattato correttamente
        ...
        for ref_kg in referenze_in_kg:
            if ref_kg in voce_lower:
                print(f"ðŸ” DEBUG determina_unita_misura per '{voce_originale}':")
                ...
                # QUANTITA_NUMERICA Ã¨ il numero di pezzi calcolato
                # Dobbiamo convertire in kg moltiplicando per peso_unitario
                kg_effettivi = quantita_numerica * peso_unitario
                print(f"   - Conversione in kg: {quantita_numerica} pezzi Ã— {peso_unitario} = {kg_effettivi:.3f} kg")
                ...
                print(f"   âœ… Risultato finale: {risultato}")
                return risultato
    ...
    def stampa_quantita_docx(self, quant, voce, cella, bold=False):
        ...
        """Stampa quantitÃ  nel documento Word con unitÃ  corretta"""
        print(f"ðŸ“ DEBUG stampa_quantita_docx per '{voce}':")
        print(f"   - quant ricevuto: {quant}")
        print(f"   - tipo quant: {type(quant)}")
        p = cella.add_paragraph()
        
        # Usa il nuovo metodo per determinare l'unitÃ 
        quantita_formattata = self.determina_unita_misura(voce, quant)
        
        # Testo finale
        # Gestione speciale per primi elaborati che giÃ  contengono il tipo
        ...
        if any(primo in quantita_formattata.lower() for primo in primi_elaborati):
            # Se la quantitÃ  giÃ  contiene il tipo, usa solo quella
    ...
    def generate_doc(self):
        ...
        # ========== DETERMINA MODALITÃ€ E CREA DOCUMENTO ==========
        doc = None
        final_path = None
        
        # Controlla se siamo in modalitÃ  aggiungi servizi
        if hasattr(self, 'modalita_aggiungi_servizi') and self.modalita_aggiungi_servizi and hasattr(self, 'ordine_da_modificare') and self.ordine_da_modificare:
            # MODALITÃ€ AGGIUNGI SERVIZI
            original_path = self.ordine_da_modificare.get("path_docx")
            if original_path and os.path.exists(original_path):
                print(f"âž• MODALITÃ€ AGGIUNGI SERVIZI: Aggiungendo al documento {original_path}")
                final_path = original_path
                
                try:
                    doc = Document(original_path)
                    print("âœ… Documento esistente caricato per aggiungi servizi")
                    ...
                    print(f"âŒ Errore caricamento documento per aggiungi servizi: {e}")
                    doc = Document()  # Crea nuovo se fallisce
                    final_path = None  # VerrÃ  creato dopo
                    
        # Controlla se siamo in modalitÃ  modifica normale
            ...
            # MODALITÃ€ MODIFICA NORMALE (sovrascrive)
            original_path = self.ordine_da_modificare.get("path_docx")
            if original_path and os.path.exists(original_path):
                final_path = original_path
                print(f"ðŸ”„ MODALITÃ€ MODIFICA: Sovrascrivendo {original_path}")
            doc = Document()  # Sempre nuovo documento per modifica
            
        else:
            # MODALITÃ€ NORMALE: Nuovo ordine
            print("ðŸ“„ MODALITÃ€ NORMALE: Nuovo ordine")
            doc = Document()

        # Se doc Ã¨ ancora None, crealo
        if doc is None:
            print("âš ï¸ Creazione documento di emergenza")
            doc = Document()
       ...
       # ========== GENERA INTESTAZIONE (solo se NON aggiungi servizi) ==========
        ...
        if not (hasattr(self, 'modalita_aggiungi_servizi') and self.modalita_aggiungi_servizi):
            ...
            # Crea tabella NR PROGR per prima (andrÃ  in centro)
        ...
        # Calcolo intelligente delle quantitÃ 
        ...
        for k, checkbox in self.selected.items():
            if hasattr(checkbox, 'isChecked') and checkbox.isChecked() and self.voce_categoria.get(k) == "cibo":
                ...
                print(f"ðŸ” CATEGORIZZANDO: k='{k}', voce='{voce_lower}'")  # QUI va il debug
                ...
                # Contorni E insalate specifiche
                    ...
                    print(f"   âž¡ï¸ CONTORNI (o insalata forzata)")
                ...
                # Finger food (tutto il resto)
                    ...
                    print(f"   âž¡ï¸ ACCOMPAGNAMENTI (pane/grissini)")
                else:
                    self.referenze_salato.append(k)
                    print(f"   âž¡ï¸ SALATO (finger food)")
        ...
        for key in self.referenze_salato:
            ...
            print(f"ðŸ”§ VALORE RESTITUITO da calcola_quantitÃ  per {voce}: {quant}")  # â¬…ï¸ AGGIUNGI QUESTO
        ...
        # 7. TORTA (grassetto) â€” ULTIMA VOCE
        for key in referenze_cibo:
            voce = self.voce_originale.get(key, key)
            if "kg torta" in voce.strip().lower():
                quant = self.calcola_quantita(key, referenze_cibo, num_pers, servizio_lower, tipo_ap)
        ...
        def add_inner_table_to_cell(testo_titolo, categoria, parent_cell):
            ...
            print(f"\nðŸ”§ Generando sezione: {testo_titolo} (categoria: {categoria})")
            
            # Trova tutte le voci per questa categoria
            voci_categoria = []
            
            # ========== FIX PER MODALITÃ€ AGGIUNGI SERVIZI ==========
            # Se modalitÃ  aggiungi servizi, includi anche scelte esistenti dall'ordine precedente
            print(f"ðŸ” CERCANDO VOCI PER CATEGORIA: {categoria}")
            for k, checkbox in self.selected.items():
                if hasattr(checkbox, 'isChecked') and checkbox.isChecked():
                    ...
                    if voce_cat == categoria:
                        if voce_nome not in voci_categoria:  # Evita duplicati
                            ...
                            print(f"    âœ… Nuova: {voce_nome}")

            # ========== AGGIUNGI DEBUG ==========
            if hasattr(self, 'modalita_aggiungi_servizi') and self.modalita_aggiungi_servizi:
                print(f"    ðŸŽ¯ MODALITÃ€ AGGIUNGI SERVIZI: Mostrando SOLO nuove {categoria}")
                print(f"    ðŸ“‹ Trovate {len(voci_categoria)} nuove voci per {categoria}")
            ...
            for voce in voci_categoria:
                inner_cell.add_paragraph(f"â€¢ {voce}")
            
            self.apply_borders(inner_cell)
            print(f"    âœ… Sezione {testo_titolo} creata con {len(voci_categoria)} voci")
            
        def add_inner_table_to_cell_menu_extra(testo_titolo, menu_type, parent_cell):
            """Crea riquadro separato per menu extra"""
            print(f"\nðŸŒ± Generando riquadro: {testo_titolo}")
            
            # Trova le voci selezionate per questo menu extra
            voci_menu = []
            for k, checkbox in self.selected.items():
                if hasattr(checkbox, 'isChecked') and checkbox.isChecked():
                    print(f"   ðŸ” Checking key: {k} (checked: {checkbox.isChecked()})")
                    if menu_type.lower() in k.lower():
                        print(f"   âœ… MATCH trovato per {k}")
                        ...
                        print(f"    âœ… {quantita_box} pz {voce}")
            
            print(f"ðŸŽ¯ Totale voci trovate: {len(voci_menu)}")
            ...
            if voci_menu:
                ...
                # Aggiungi tutte le voci con quantitÃ 
                for voce_con_quantita in voci_menu:
                    inner_cell.add_paragraph(f"â€¢ {voce_con_quantita}")
                
                self.apply_borders(inner_cell)
                print(f"    âœ… Riquadro {testo_titolo} creato con {len(voci_menu)} voci")
        # ========== CONTROLLA SE CI SONO BEVANDE (esistenti + nuove) ==========
        has_bevande = False
        
        # In modalitÃ  aggiungi servizi, controlla anche bevande esistenti
        if hasattr(self, 'modalita_aggiungi_servizi') and self.modalita_aggiungi_servizi and hasattr(self, 'ordine_da_modificare') and self.ordine_da_modificare:
        ...
        # In modalitÃ  aggiungi servizi, controlla anche accessori esistenti
        if hasattr(self, 'modalita_aggiungi_servizi') and self.modalita_aggiungi_servizi and hasattr(self, 'ordine_da_modificare') and self.ordine_da_modificare:
            scelte_esistenti = self.ordine_da_modificare.get("scelte", {})
        ...
        if menu_extra_attivi:
            ...
            for i, menu_type in enumerate(menu_types):
                ...
                # Debug
                print(f"ðŸŒ± Riquadro {menu_type}: {len(voci_trovate)} voci in posizione [{row_index}, {col_index}]")
        ...
        doc.save(final_path)
        print(f"ðŸ“„ Documento salvato: {final_path}")
                ...
                if widget.value() > 0:
                    ...
                    for key, checkbox in self.selected.items():
                        if (hasattr(checkbox, 'isChecked') and checkbox.isChecked() and 
                            ...
                            print(f"ðŸŒ± FIXED: {voce} â†’ {quantita_raw} â†’ {quantita_finale} pz")
        ...
        print(f"ðŸŒ± Intolleranze da salvare: {intolleranze}")
        ...
        ordine = {
            ...
            "intolleranze": intolleranze  # â† AGGIUNGI QUESTA RIGA
        }

        # AGGIUNGI QUESTA RIGA DI DEBUG:
        print("ðŸŽ¯ === ORDINE CREATO, DEVO CHIAMARE SALVA_ORDINE ===")

        # POI AGGIUNGI QUI LA CHIAMATA:
        if hasattr(self, 'modalita_aggiungi_servizi') and self.modalita_aggiungi_servizi:
            print("ðŸ’¾ Aggiornando ordine esistente con nuovi servizi")
            self.aggiorna_ordine_esistente(ordine)
        else:
            print("ðŸ’¾ Salvando ordine...")
            self.salva_ordine(ordine)

        # ========== SALVA PRIMA DI RESETTARE LE MODALITÃ€ ==========
        nome = self.entries["Nome"].text()
        if hasattr(self, 'modalita_aggiungi_servizi') and self.modalita_aggiungi_servizi:
        ...
        if hasattr(self, 'modalita_modifica'):
            print(f"ðŸ”„ Resettando modalita_modifica da {self.modalita_modifica} a False")
            self.modalita_modifica = False
        ...
        print("âœ… Generazione completata e modalitÃ  resettate")
    ...
    def debug_voce_categoria(self):
        ...
        print("\nðŸ” === DEBUG CATEGORIZZAZIONE VOCI ===")
        ...
        print("ðŸ“‹ Accessori che dovrebbero essere in SALA:")
        for acc in accessori_previsti:
            ...
            for key, categoria in self.voce_categoria.items():
                ...
                if acc.lower() in voce:
                    print(f"  â€¢ {acc} â†’ Categoria: {categoria} (Key: {key[:50]}...)")
                    found = True
                    break
            if not found:
                print(f"  âŒ {acc} â†’ NON TROVATO!")
        
        print("\nðŸ“Š Tutte le voci categorizzate come 'accessori':")
        for key, categoria in self.voce_categoria.items():
            if categoria == "accessori":
                voce = self.voce_originale.get(key, key)
                print(f"  âœ… {voce} (Key: {key[:50]}...)")
    ...
    def aggiungi_al_documento_esistente(self):
        ...
        reply = QMessageBox.question(
            ...
            f"Il nuovo servizio verrÃ  aggiunto al documento Word esistente.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
        ...
        # Usa la funzione generate_doc normale, che gestirÃ  la modalitÃ 
        self.generate_doc()
    ...
    def reset_modalita_aggiungi_servizi(self):
        ...
        print("âœ… Interfaccia resettata alla modalitÃ  normale")

    def set_ap_tipo(self, tipo):
        self.ap_buffet_tipo = tipo
        print(f"ðŸŽ¯ Tipo aperitivo impostato: '{tipo}'")  # Debug
    # ========== FUNZIONI HELPER (da aggiungere alla classe) ==========
    
    # SOLUZIONE PROBLEMA 2: Correzione quantitÃ  per Coffee/Tea Break e Aperitivo Rinforzato

    def calcola_quantita_menu_extra_simple(self, voce, persone, servizio_principale=None):
        """Calcolo quantitÃ  per menu extra - VERSIONE CORRETTA CON QUANTITÃ€ GIUSTE"""
        
        print(f"\nðŸŒ± === CALCOLO MENU EXTRA SEMPLICE ===")
        print(f"   ðŸ“ Voce: {voce}")
        print(f"   ðŸ‘¥ Persone: {persone}")
        ...
        if is_primo:
            print(f"   ðŸ PRIMO: 1 per persona")
            return f"({persone})1"
        
        # ========== GESTIONE DOLCI ==========
        # ========== GESTIONE DOLCI ==========
        is_dolce = any(parola in voce_lower for parola in ["dolc", "torta", "crostat", "tiramisu", "mousse", "spicchietti", "frutta"])
        if is_dolce:
            # USA IL SERVIZIO PASSATO COME PARAMETRO
            if servizio_principale:
                servizio_lower = servizio_principale.lower()
                print(f"   ðŸ½ï¸ Servizio per dolci: {servizio_principale}")
            else:
                ...
                print(f"   ðŸ½ï¸ Servizio fallback per dolci: {servizio}")
            ...
            if "aperitivo" in servizio_lower and "rinforzato" in servizio_lower:
                ...
                print(f"   ðŸ° APERITIVO RINFORZATO - DOLCI: {dolci_per_persona} per persona")
                return f"({dolci_totali}){dolci_per_persona}"
            else:
                print(f"   ðŸ° DOLCE STANDARD: 1 per persona")
                return f"({persone})1"
        ...
        if servizio_principale:
            servizio_lower = servizio_principale.lower()
            print(f"   ðŸ½ï¸ Servizio da parametro: {servizio_principale}")
        else:
            # Fallback alla logica originale
            servizio = self.entries.get("Servizio", "").strip() if "Servizio" in self.entries else ""
            print(f"   ðŸ” DEBUG Servizio field: '{servizio}'")
            
            if not servizio:
                tipo_ap = getattr(self, 'ap_buffet_tipo', '').lower()
                print(f"   ðŸ” DEBUG tipo_ap: '{tipo_ap}'")
            ...
            servizio_lower = servizio.lower()
            print(f"   ðŸ½ï¸ Servizio finale fallback: {servizio}")
        ...
        if not menu_type_trovato:
            print(f"   âš ï¸ Menu type non trovato")
            return f"({persone})1"
        
        print(f"   ðŸ“‹ Menu type: {menu_type_trovato}")
        ...
        for key, checkbox in self.selected.items():
            if (hasattr(checkbox, 'isChecked') and checkbox.isChecked() and 
                ...
                # Verifica se Ã¨ un primo nel menu intolleranti
                if any(parola in voce_check_lower for parola in ["pasta corta", "pasta al forno"]):
                    ha_primi_menu_intolleranti = True
                    print(f"      âœ… PRIMO nel menu intolleranti: {voce_check}")
                    break

        print(f"   ðŸ Ha primi nel menu INTOLLERANTI: {ha_primi_menu_intolleranti}")
        ...
        for key, checkbox in self.selected.items():
            if (hasattr(checkbox, 'isChecked') and checkbox.isChecked() and 
                ...
                if not is_dolce_check and not is_primo_check:
                    voci_salate_count += 1
                    print(f"      âœ… Voce salata: {voce_check}")
        
        print(f"   ðŸ“Š Totale voci salate: {voci_salate_count}")
        ...
        if "coffee" in servizio_lower or "tea" in servizio_lower:
            ...
            print(f"   â˜• COFFEE/TEA: 2 finger food salati per persona (CORRETTO)")
            
        elif "aperitivo" in servizio_lower and "leggero" in servizio_lower:
            finger_food_per_persona = 6
            print(f"   ðŸ¥‚ APERITIVO LEGGERO: 6 finger food per persona")
            ...
            print(f"   ðŸ¥‚ APERITIVO RINFORZATO: 8 finger food salati per persona (CORRETTO)")
            
        elif "lunch" in servizio_lower or "pranzo" in servizio_lower:
            if ha_primi_menu:
                finger_food_per_persona = 4
                print(f"   ðŸ½ï¸ LUNCH CON PRIMI: 4 finger food per persona")
            else:
                finger_food_per_persona = 8
                print(f"   ðŸ½ï¸ LUNCH SENZA PRIMI: 8 finger food per persona")
        else:
            # Default: considera come lunch
            if ha_primi_menu:
                finger_food_per_persona = 2
                print(f"   ðŸ½ï¸ DEFAULT CON PRIMI: 4 finger food per persona")
            else:
                finger_food_per_persona = 8
                print(f"   ðŸ½ï¸ DEFAULT SENZA PRIMI: 8 finger food per persona")
                
        # ========== OVERRIDE FINALE: Se non Ã¨ nÃ© coffee nÃ© aperitivo, forza logica lunch ==========
        if not ("coffee" in servizio_lower or "tea" in servizio_lower or "aperitivo" in servizio_lower):
            if ha_primi_menu:
                finger_food_per_persona = 4
                print(f"   ðŸ”§ OVERRIDE FINALE: 4 finger food (con primi)")
            else:
                finger_food_per_persona = 8
                print(f"   ðŸ”§ OVERRIDE FINALE: 8 finger food (senza primi)")
        ...
        if voci_salate_count == 1:
            ...
            print(f"   ðŸ“Š 2 voci: {finger_food_per_persona} Ã· 2 = {pezzi_per_persona} per voce")
            ...
            if finger_food_per_persona == 8:
                ...
                voci_ordinate.sort()
                print(f"   ðŸ” DEBUG 3 VOCI (8 finger): voci ordinate = {voci_ordinate}")
                print(f"   ðŸ” DEBUG: voce corrente = '{voce}'")
                
                if voce in voci_ordinate:
                    indice = voci_ordinate.index(voce)
                    print(f"   ðŸ” DEBUG: indice voce = {indice}")
                    if indice == 0:
                        pezzi_per_persona = 3  # Prima voce: 3
                        print(f"   âœ… Prima voce: 3 pezzi")
                    elif indice == 1:
                        pezzi_per_persona = 3  # Seconda voce: 3
                        print(f"   âœ… Seconda voce: 3 pezzi")
                    else:
                        pezzi_per_persona = 2  # Terza voce: 2
                        print(f"   âœ… Terza voce: 2 pezzi")
                else:
                    pezzi_per_persona = 3
                    print(f"   âš ï¸ Voce non trovata in lista, default 3")
        ...
        print(f"   ðŸŽ¯ Distribuzione {finger_food_per_persona} finger food tra {voci_salate_count} voci")
        print(f"   ðŸ“Š Pezzi per persona calcolati: {pezzi_per_persona}")
        print(f"   ðŸ“Š Pezzi totali: {pezzi_totali}")
        print(f"   ðŸ” DEBUG: voce corrente = '{voce}'")
        print(f"   ðŸ” DEBUG: finger_food_per_persona = {finger_food_per_persona}")
        print(f"   ðŸ” DEBUG: voci_salate_count = {voci_salate_count}")
    ...
    def calcola_quantita_menu_extra_dolci_aperitivo_rinforzato(self, voce, persone):
        """Calcolo speciale per dolci nell'aperitivo rinforzato: 3 dolci per persona"""
        
        servizio = self.entries.get("Servizio", "").strip().lower()
        tipo_ap = getattr(self, 'ap_buffet_tipo', '').lower()
        
        # Se Ã¨ aperitivo rinforzato, i dolci sono 3 per persona
        if "aperitivo" in servizio and "rinforzato" in tipo_ap:
            dolci_per_persona = 3
            dolci_totali = dolci_per_persona * persone
            print(f"   ðŸ° APERITIVO RINFORZATO - DOLCI: {dolci_per_persona} per persona")
            return f"({dolci_totali}){dolci_per_persona}"
        
        # Altrimenti usa la logica standard (1 dolce per persona)
        return f"({persone})1"
    ...
    def aggiorna_ordine_esistente(self, nuovo_ordine):
        ...
        print(f"\nðŸ”§ === AGGIORNA_ORDINE_ESISTENTE ===")
        print(f"ðŸ“‚ File: {ORDINI_FILE}")
        
        if not os.path.exists(ORDINI_FILE):
            print("âŒ File archivio non esiste, uso salva_ordine normale")
        ...
        print(f"ðŸ“‹ Caricati {len(ordini)} ordini esistenti")
        ...
        if hasattr(self, 'indice_ordine_originale') and self.indice_ordine_originale is not None:
            ...
            if 0 <= idx < len(ordini):
                print(f"ðŸŽ¯ Usando indice memorizzato: {idx}")
                ...
                print(f"ðŸ” SCELTE ESISTENTI: {sum(1 for v in scelte_esistenti.values() if v)} voci")
                print(f"ðŸ” SCELTE NUOVE: {sum(1 for v in scelte_nuove.values() if v)} voci")
                ...
                for k, v in scelte_nuove.items():
                    if v:  # Solo se la nuova scelta Ã¨ selezionata
                        scelte_combinate[k] = True

                print(f"ðŸ” SCELTE COMBINATE: {sum(1 for v in scelte_combinate.values() if v)} voci")
                ...
                try:
                    ...
                    for campo, valore in campi_nuovi.items():
                        ...
                        elif valore:  # Solo se il nuovo valore non Ã¨ vuoto
                            campi_aggiornati[campo] = valore
                    
                    ordini[idx]["campi"] = campi_aggiornati
                    print(f"âœ… Persone aggiornate: {persone_esistenti} + {persone_nuove} = {persone_totali}")
                    
                except (ValueError, TypeError):
                    # Se c'Ã¨ un errore, usa il comportamento originale
                    ordini[idx]["campi"] = nuovo_ordine["campi"]
                    print("âš ï¸ Errore nella somma, uso comportamento originale")
                ...
                ordine_trovato = True
                print(f"âœ… Ordine combinato e aggiornato in posizione {idx}")
                print(f"ðŸ“Š Scelte totali dopo combinazione: {len(scelte_combinate)}")
        
        # STRATEGIA 2: Fallback per path
        if not ordine_trovato:
            path_da_cercare = nuovo_ordine["path_docx"]
            print(f"ðŸ” Cercando per path: {path_da_cercare}")
            ...
            for i, ordine in enumerate(ordini):
                if ordine.get("path_docx") == path_da_cercare:
                    ...
                    ordine_trovato = True
                    print(f"âœ… Ordine trovato per path e aggiornato in posizione {i}")
                    break
        
        if not ordine_trovato:
            print("âŒ Ordine non trovato, aggiungendo come nuovo")
            ordini.append(nuovo_ordine)
            print(f"âž• Nuovo ordine aggiunto")
        ...
        try:
            ...
            print(f"ðŸ’¾ File salvato con {len(ordini)} ordini totali")
        ...
        # ========== QUESTA Ãˆ LA PARTE MANCANTE - AGGIORNA ARCHIVI APERTI ==========
        global archivio_dialogs_aperti
        print(f"ðŸ“Š Archivi aperti da aggiornare: {len(archivio_dialogs_aperti)}")
        ...
        if hasattr(self, 'indice_ordine_originale'):
            ...
            print("ðŸ§¹ Indice ordine originale resettato")

    def carica_dati_per_aggiungi_servizi(self, ordine):
        """Carica dati cliente e azzera scelte - VERSIONE CORRETTA"""
        print(f"ðŸ”„ ModalitÃ  aggiungi servizi per: {ordine['campi'].get('Nome', 'Cliente')}")
        ...
        print("âœ… Dati caricati, scelte azzerate")
        ...
        if hasattr(self, 'aggiungi_ordine_btn') and hasattr(self, 'genera_btn'):
            ...
            # Aggiorna il titolo della finestra per indicare la modalitÃ 
            self.setWindowTitle("Neri Gestione Catering - AGGIUNGI SERVIZI")
        
        print("âœ… Interfaccia aggiornata per modalitÃ  aggiungi servizi")
...
def apri_documento_word(self, file_path):
    ...
    try:
        ...
        print(f"ðŸ“„ Documento aperto automaticamente: {file_path}")
        
    except Exception as e:
        print(f"âŒ Errore apertura documento: {e}")
        QMessageBox.information(self, "Info", 
                               f"Documento salvato in:\n{file_path}\n\n"
                               f"Aprilo manualmente per modificare le quantitÃ .")
...
def is_header_or_non_quantity(self, riga):
    """Controlla se una riga Ã¨ un header o non contiene quantitÃ """
    ...
    header_keywords = [
        ...
        "bevande", "sala", "lunedÃ¬", "martedÃ¬", "mercoledÃ¬", "giovedÃ¬", 
        "venerdÃ¬", "sabato", "domenica", "gennaio", "febbraio", "marzo",
    ...
    # Se Ã¨ troppo corta (meno di 3 caratteri), salta
    if len(riga.strip()) < 3:
        return True
        
    # Se Ã¨ solo punteggiatura, salta
    if riga.strip() in ["â€¢", "-", "*", "â—‹", "â—"]:
        return True
    
    return False

def estrai_quantita_da_riga_migliorata(self, riga):
    """Estrae quantitÃ  da una singola riga - VERSIONE MIGLIORATA"""
    import re
    ...
    patterns = [
        # Pattern per peso in kg (piÃ¹ specifico)
        (r'^(\d+\.?\d*)\s*kg\s+(.+)$', "kg"),
        
        # Pattern per lasagne, crespelle, etc.
        (r'^(\d+)\s+(lasagne|crespelle|cannelloni|rosette|nidi)(?:\s+(.+))?$', "primi"),
        
        # Pattern generico numero + nome (piÃ¹ permissivo)
        (r'^(\d+)\s+(.+)$', "pezzi"),
        
        # Pattern per quantitÃ  con **grassetto** (primi piatti)
    ...
    for pattern, tipo in patterns:
        ...
        if match:
            try:
                ...
                print(f"        âœ… Match {tipo}: '{riga_originale}' â†’ {nome_prodotto}: {quantita_str}")
                break
                
            except (ValueError, AttributeError) as e:
                print(f"        âŒ Errore parsing: {e}")
                continue
...
def pulisci_nome_prodotto(self, nome):
    ...
    nome = nome.replace("â€¢", "").replace("â—‹", "").replace("â—", "").replace("-", "", 1)
...
def trova_chiave_corrispondente_debug(self, nome_prodotto):
    ...
    nome_lower = nome_prodotto.lower()
    print(f"            ðŸ” Ricerca chiave per: '{nome_prodotto}' (lower: '{nome_lower}')")
    
    print(f"            ðŸ“‹ Voci disponibili nel menu:")
    for key, voce_originale in self.voce_originale.items():
        print(f"               {key} â†’ '{voce_originale}'")
    ...
    for key, voce_originale in self.voce_originale.items():
        ...
        if nome_lower == voce_lower:
            print(f"            âœ… MATCH DIRETTO: '{nome_lower}' == '{voce_lower}'")
            return key
    ...
    for key, voce_originale in self.voce_originale.items():
        ...
        if nome_lower in voce_lower:
            print(f"            âœ… MATCH PARZIALE: '{nome_lower}' IN '{voce_lower}'")
            if len(nome_lower) >= 4:  # Evita match troppo corti
                return key
        
        if voce_lower in nome_lower:
            print(f"            âœ… MATCH PARZIALE: '{voce_lower}' IN '{nome_lower}'")
            if len(voce_lower) >= 4:
                return key
    ...
    if "pan" in nome_lower and "chocolat" in nome_lower:
        for key, voce_originale in self.voce_originale.items():
            ...
            if "pan" in voce_lower and "chocolat" in voce_lower:
                print(f"            âœ… MATCH SPECIFICO PAN CHOCOLAT: {key}")
                return key
    ...
    for nome_mapping, parole_chiave in mapping_specifici.items():
        if all(parola in nome_lower for parola in parole_chiave):
            for key, voce_originale in self.voce_originale.items():
                voce_lower = voce_originale.lower()
                if all(parola in voce_lower for parola in parole_chiave):
                    print(f"            âœ… MATCH MAPPING: {nome_mapping} â†’ {key}")
                    return key
...
def test_parsing_mini_pan_chocolat(self):
    ...
    print("\nðŸ§ª === TEST PARSING MINI PAN CHOCOLAT ===")
...
def main():
    ...
    def start_main_application():
        ...
        print("âœ… Applicazione principale avviata")
