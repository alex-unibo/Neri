...
# SOLUZIONE: Migliorare il metodo leggi_quantita_da_word per leggere TUTTO
    ...
    def calcola_bilanciamento_dinamico(self, servizio_lower, tipo_ap, num_pers):
        ...
        riduzione_finger = riduzione_totale * 0.2
    ...
    def calcola_quantita(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap):
        ...
        # GESTIONE ESPECIAL PER APERITIVO (senza bilanciamento se non è rinforzato)
        # GESTIONE SPECIALE PER APERITIVO (distingui dolci da salati)
        if "aperitivo" in servizio_lower and key not in self.referenze_primi:
    
            # Verifica se è un dolce
            ...
            if is_dolce and tipo_ap == "rinforzato":
                # Usa la funzione specifica per dolci dell'aperitivo rinforzato
                ...
                # Usa la logica originale per finger food salati
                print(f"🥂 FINGER FOOD APERITIVO: chiamando funzione originale")
                return self._calcola_aperitivo_originale(key, referenze_cibo, num_pers, servizio_lower, tipo_ap)
        ...
        # GESTIONE SECONDI (quantità ridotte per lunch e aperitivo rinforzato)
        # GESTIONE SECONDI (quantità ridotte per lunch e aperitivo rinforzato)
...
# ==============================================================================
    ...
    def _calcola_primi_con_bilanciamento(self, key, num_pers, servizio_lower, tipo_ap, bilanciamento, peso_unitario):
        ...
        if is_servito_service:
            peso_primi_base_per_persona = 0.220  # 220g per servito
    ...
    def _calcola_aperitivo_originale(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap):
        """Logica per aperitivo - SOLO FINGER FOOD SALATI (esclude dolci dal calcolo)"""
        voce = self.voce_originale.get(key, key).strip()
        ...
        # PESO SOLO per finger food salati (esclusi i dolci)
        if tipo_ap == "rinforzato":
            peso_salati_persona = 0.260  # 280g salati
        else:  # leggero
            peso_salati_persona = 0.165  # 230g totali - 40g dolci = 190g salati
        
        peso_totale_salati = num_pers * peso_salati_persona
        
        # CALCOLA COEFFICIENTI SOLO PER FINGER FOOD SALATI (esclude i dolci)
        ...
        for k in referenze_cibo:
            ...
            # ESCLUDI i dolci dal calcolo - solo finger food salati
            ...
            # ESCLUDI anche bruschettine e crostini se sono stati classificati come dolci
            if ("bruschettine" in voce_k or "crostini" in voce_k) and not is_dolce:
                # Questi sono finger food salati, li includiamo
                pass
            elif is_dolce:
                continue  # Salta i dolci
        ...
        print(f"   - Peso SALATI per persona: {peso_salati_persona*1000}g")
        print(f"   - Peso totale salati: {peso_totale_salati:.3f} kg")
        print(f"   - Referenze salati considerate: {len(referenze_salati)} (esclusi dolci)")
        ...
        peso_target = peso_totale_salati * quota
    ...
    def _calcola_finger_food_con_bilanciamento(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap, peso_unitario, bilanciamento):
        ...
        if use_balancing:
            ...
            if "coffee" in servizio_lower or "tea" in servizio_lower:
                ...
                peso_salato_per_persona = 0.200  # 200g per servito e altri servizi
    ...
    def gestisci_selezione_insalata(self, checkbox, key_insalata):
        ...
        # Verifica se questa voce è una delle insalate speciali
    ...
    def calcola_quantita_menu_extra_dolci_aperitivo_rinforzato(self, voce, persone, servizio_lower, tipo_ap):
        ...
        if "aperitivo" in servizio_lower and "rinforzato" in tipo_ap:
            print(f"   ✅ ENTRANDO NELLA LOGICA APERITIVO RINFORZATO!")
            
            # Calcola peso totale dolci target
            peso_dolci_totale = 0.070 * persone  # 70g per persona
            
            # Conta quanti dolci sono selezionati
            dolci_selezionati = 0
            for key, checkbox in self.selected.items():
                if hasattr(checkbox, 'isChecked') and checkbox.isChecked():
                    voce_check = self.voce_originale.get(key, key.split('|')[-1]).lower()
                    # Verifica se è un dolce
                    is_dolce = any(parola in voce_check for parola in ["dolc", "torta", "crostat", "tiramisu", "mousse", "spicchietti", "frutta", "biscotteria", "pasticceria"])
                    if is_dolce:
                        dolci_selezionati += 1
                        print(f"      - Dolce trovato: {voce_check}")
            
            if dolci_selezionati == 0:
                print(f"   🍰 APERITIVO RINFORZATO - Nessun dolce selezionato")
                return f"({persone})1"
            
            # Calcola peso target per questo dolce (distribuzione uniforme)
            peso_per_dolce = peso_dolci_totale / dolci_selezionati
            
            # Ottieni peso unitario per questa voce specifica
            voce_lower = voce.lower()
            peso_unitario = self.dati_referenze.get(voce_lower, {"peso_unitario": 0.03333})["peso_unitario"]
            
            # Calcola pezzi necessari
            pezzi_necessari = peso_per_dolce / peso_unitario
            pezzi_finali = max(1, int(round(pezzi_necessari)))
            
            print(f"   🍰 APERITIVO RINFORZATO - DOLCI:")
            print(f"      - Peso totale dolci: {peso_dolci_totale:.3f} kg (70g/persona)")
        ...
        print(f"   🍰 DOLCE STANDARD: 1 per persona (condizione aperitivo rinforzato non soddisfatta)")
        return f"({persone})1"
