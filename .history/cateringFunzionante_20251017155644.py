...
# ==============================================================================
    ...
    def _calcola_finger_food_con_bilanciamento(self, key, referenze_cibo, num_pers, servizio_lower, tipo_ap, peso_unitario, bilanciamento):
        ...
        if use_balancing:
            ...
            if "coffee" in servizio_lower or "tea" in servizio_lower:
                # SEPARAZIONE TRA DOLCI E SALATI PER COFFEE/TEA BREAK
                voce_lower = voce.lower()
                is_dolce = any(parola in voce_lower for parola in ["dolc", "torta", "crostat", "tiramisu", "mousse",
                                                                     "spicchietti", "frutta", "biscotteria", "pasticceria",
                                                                     "fagottini", "pan au chocolat", "brioche"])

                if is_dolce:
                    peso_per_persona = 0.040  # 40g per dolci
                    print(f"   ☕ Coffee/Tea break DOLCI: 40g per persona")
                else:
                    peso_per_persona = 0.0333  # 33g per salati
                    print(f"   ☕ Coffee/Tea break SALATI: 33g per persona")

                # Calcola quota per questa voce separando dolci da salati
                pesi_difficalta = []
                referenze_finger_stessa_categoria = []
                ...
                for k in referenze_cibo:
                    ...
                    # Verifica se è dolce
                    is_dolce_k = any(parola in voce_k for parola in ["dolc", "torta", "crostat", "tiramisu", "mousse",
                                                                       "spicchietti", "frutta", "biscotteria", "pasticceria",
                                                                       "fagottini", "pan au chocolat", "brioche"])

                    # Includi solo prodotti della stessa categoria (dolci con dolci, salati con salati)
                    if is_dolce == is_dolce_k:
                        referenze_finger_stessa_categoria.append(k)
                        dati_k = self.dati_referenze.get(voce_k, {"difficolta": 1.0})
                        pesi_difficalta.append(dati_k["difficolta"])

                # Usa solo le referenze della stessa categoria per il calcolo
                if referenze_finger_stessa_categoria:
                    somma_coeff = sum(pesi_difficalta)
                    quota = coeff / somma_coeff if somma_coeff > 0 else 1 / len(referenze_finger_stessa_categoria)
                else:
                    quota = 1.0
                ...
                print(f"   🍽️ Coffee/Tea Break - Categoria: {'Dolci' if is_dolce else 'Salati'}")
                print(f"   🍽️ Peso target: {peso_target:.3f} kg")
                print(f"   🍽️ Quota: {quota:.3f} (tra {len(referenze_finger_stessa_categoria)} prodotti)")
                ...
                print(f"   🍽️ Aperitivo leggero - Peso target: {peso_target:.3f} kg")

            else:
                peso_salato_per_persona = 0.450
                print(f"   🍽️ Altri servizi: 450g per persona")
                ...
                print(f"   🍽️ Altri servizi - Peso target: {peso_target:.3f} kg")
            ...
            if self._va_in_kg(voce):
                ...
                print(f"   🍽️ Finger food finale: {risultato_pezzi} pezzi")
                return f"{risultato_pezzi} pz"
