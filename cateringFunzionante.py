...
""")
    ...
    def setup_ui(self):
        ...
        self.scroll_area.verticalScrollBar().setSingleStep(40)  # Step più fluido
        self.scroll_area.verticalScrollBar().setPageStep(200)   # Page step ottimizzato
    ...
    def create_chart_improved(self, servizi_count):
        ...
        # Funzione per passare gli eventi wheel al parent scroll area in modo più pulito
        original_wheel_event = canvas.wheelEvent
        def wheelEvent(event):
            # Passa l'evento wheel direttamente al scroll area parent
            if hasattr(self, 'scroll_area') and self.scroll_area:
                # Crea un nuovo evento per lo scroll area
...
class CateringApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Ottimizza lo scroll
        main_scroll.verticalScrollBar().setSingleStep(40)
        main_scroll.verticalScrollBar().setPageStep(200)
    ...
    def add_service_tab(self, name, sections):
        ...
        scroll.verticalScrollBar().setPageStep(200)
...
# SOLUZIONE: Migliorare il metodo leggi_quantita_da_word per leggere TUTTO
    ...
    def _calcola_primi_lunch_buffet(self, key, num_pers, voce, voce_lower):
        """Calcola quantità primi per lunch buffet con bilanciamento"""

        # Peso base primi per lunch buffet: 222g per persona
        peso_primi_base_per_persona = 0.180
        peso_primi_totale_finale = peso_primi_base_per_persona * num_pers

        print(f"   🍝 LUNCH BUFFET PRIMI - peso base: 222g per persona")
    ...
    def _calcola_lunch_buffet(self, key, referenze_cibo, num_pers, servizio_lower):
        """
        Calcola quantità per Lunch Buffet con logica bilanciamento primi.

        LOGICA:
        CON PRIMI (260g totali):
        - Salati: 180g per persona
        - Dolci: 80g per persona

        SENZA PRIMI (380g totali):
        - Salati: 300g per persona
        - Dolci: 80g per persona

        - Primi elaborati (lasagne, crespelle, ecc.): calcolo fisso
        ...
        # Verifica se ci sono primi selezionati
        ...
        # Determina pesi in base alla presenza di primi
        if ha_primi:
            # CON PRIMI: finger food + dolci
            if is_dolce:
                peso_per_persona = 0.080  # 80g per dolci
                categoria = "DOLCI"
                print(f"   🍽️ Lunch Buffet DOLCI (con primi): 80g per persona")
            else:
                peso_per_persona = 0.190  # 180g per salati
                categoria = "SALATI"
                print(f"   🍽️ Lunch Buffet SALATI (con primi): 180g per persona")
        else:
            # SENZA PRIMI: 2 categorie
            if is_dolce:
                peso_per_persona = 0.080  # 80g per dolci
                categoria = "DOLCI"
                print(f"   🍽️ Lunch Buffet DOLCI (senza primi): 80g per persona")
            else:
                peso_per_persona = 0.330  # 300g per salati
                categoria = "SALATI"
                print(f"   🍽️ Lunch Buffet SALATI (senza primi): 300g per persona")
        ...
        for k in referenze_cibo:
            ...
            # Saltiamo i primi (già gestiti separatamente)
            if k_is_primo:
                continue
            ...
            elif not is_dolce and not k_is_dolce:
                stessa_categoria = True
        ...
        if tipo_ap == "rinforzato":
            ...
            if is_dolce:
                ...
                if not ha_dolci:
                    ...
                    peso_per_persona = 0.210  # 210g per salati (con dolci)
        ...
        if checkbox.isChecked():
            ...
            for k, cb in self.selected.items():
                if hasattr(cb, 'isChecked') and self.voce_categoria.get(k) == "accessori":
                    ...
                    # Bowl+ice per tutte le bevande con CONS o per Prosecco
                    if ("bowl" in voce_accessorio and "ice" in voce_accessorio) or "bowl+ice" in voce_accessorio:
                        if "+cons" in voce_bevanda or "prosecco" in voce_bevanda:
                            cb.setChecked(True)
                            print(f"✅ Auto-selezionato: {self.voce_originale.get(k, k)} (bevanda CONS/Prosecco, sezione: {sezione_accessorio})")

                    # Flutes per Prosecco
                    if "prosecco" in voce_bevanda and "flutes" in voce_accessorio:
                        cb.setChecked(True)
                        print(f"✅ Auto-selezionato: {self.voce_originale.get(k, k)} (per Prosecco, sezione: {sezione_accessorio})")
                        
                    # Flutes per Prosecco
                    if "pignoletto" in voce_bevanda and "flutes" in voce_accessorio:
                        cb.setChecked(True)
                        print(f"✅ Auto-selezionato: {self.voce_originale.get(k, k)} (per Prosecco, sezione: {sezione_accessorio})")
                    
                    # Calici per Chardonnay, Cabernet e Sangiovese
                    if ("chardonnay" in voce_bevanda or "cabernet" in voce_bevanda or "sangiovese" in voce_bevanda) and "calici" in voce_accessorio and "mono" in voce_accessorio:
                        cb.setChecked(True)
