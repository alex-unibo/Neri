...
class GestorePercorsi:
    ...
    def configura_percorsi_file(self):
        ...
        self.percorsi_file = {
            ...
            "logo": os.path.join(base, "neri.ico"),
        }
...
# ==============================================================================
    ...
    def copia_voci_selezionate(self):
        """Estrae le voci selezionate, le formatta con i tab e le copia negli Appunti (Clipboard)"""
        testo_formattato = ""
        voci_trovate = False

        # Raccogliamo tutte le voci spuntate
        # Raccogliamo tutte le voci spuntate
        for key, checkbox in self.selected.items():
            if hasattr(checkbox, 'isChecked') and checkbox.isChecked():
                voce = self.voce_originale.get(key, "")
                # Copia solo il nome della referenza, andando a capo per ognuna
                testo_formattato += f"{voce}\n"
                voci_trovate = True

        if voci_trovate:
            # Copia effettivamente nella memoria del computer (Appunti)
            ...
            QMessageBox.information(
                self, 
                "Copia Riuscita", 
                "✅ Voci copiate negli appunti!\n\nOra puoi premere CTRL+V (o Incolla) direttamente nel tuo documento Word o Excel."
            )
        else:
            QMessageBox.warning(self, "Attenzione", "Nessuna referenza selezionata da copiare.")
